...
!   the <a href="../module/modbase_chrstr.html">ModBase_ChrStr</a> module.  However,
...
!   Therefore, all operations provided for the *Assignable*, the *Comparable* or
...
!** DERIVED TYPE DEFINITIONS
    ...
    CONTAINS
        ...
        PROCEDURE   :: CopyAssign       => FvlStr_CopyAssign
        ...
        PROCEDURE   :: MakeClone        => FvlStr_Clone
        !> *IsEqualTo* is a procedure deferred by the *Assignable* type. <br>
        !  Use relational operators to compare two FvlStr objects (i.e. use the
        ...
        !  pointer component(s).
        PROCEDURE   :: FreeMemory       => FvlStr_Free
        ...
        PROCEDURE   :: GetTypeName      => FvlStr_GetTypeName
        ! ---------------------------------------------------------------------
        ...
        ! ---------------------------------------------------------------------
        ! -----         Deferred Procedures from Hashable Type          -----
        ! ---------------------------------------------------------------------
        !> *ComputeHashValue* is a procedure deferred by the *Hashable* type. <br>
        !  Use the *HashCode* method in place of the *ComputeHashValue* method to
        !  compute hash code of a *Hashable* object.
        PROCEDURE   :: ComputeHashValue => FvlStr_HashFunc
        ! ---------------------------------------------------------------------
        ...
        !       <a href="../module/modbase_chrstr.html#interface-ischaracterinclass">
        !       IsCharacterInClass</a> function interface and the
        !       <a href="../module/modbase_chrstr.html#interface-isstringinclass">
        !       IsStringInClass</a> function interface of the
        !       <a href="../module/modbase_chrstr.html">ModBase_ChrStr</a> module. <br>
        ...
        !       <a href="../module/modbase_chrstr.html#variable-set_blanks">
...
!** INTERFACE DEFINITIONS:
    ...
    !------------------------------------------------------------------
    ! interfaces for 'SubClass_FvlStr_Conversion'
    INTERFACE
        ! ---------------------------------------------------------------------
        ...
        ! ---------------------------------------------------------------------
    END INTERFACE
    ! interfaces for 'SubClass_FvlStr_Inquiry'
    INTERFACE
        ! ---------------------------------------------------------------------
        ...
        ! ---------------------------------------------------------------------
    END INTERFACE
    ! interfaces for 'SubClass_FvlStr_Manipulation'
    INTERFACE
        ! ---------------------------------------------------------------------
        ...
        MODULE SUBROUTINE RemoveCharacters_VLS(vStr,ChrSet,Option,Protect,ExclMrk)
            ...
            CLASS(FvlStr),      INTENT(INOUT)   :: vStr
            TYPE(FvlStr),       INTENT(IN)      :: ChrSet
            tInteger, OPTIONAL, INTENT(IN)      :: Option
            tLogical, OPTIONAL, INTENT(IN)      :: Protect
            tLogical, OPTIONAL, INTENT(IN)      :: ExclMrk
        ...
        ! ---------------------------------------------------------------------
    END INTERFACE
    ! interfaces for 'SubClass_FvlStr_Miscellaneous'
    INTERFACE
        ! ---------------------------------------------------------------------
...
SUBROUTINE FvlStr_CopyAssign(DstObj, SrcObj)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(FvlStr),     INTENT(OUT)  :: DstObj   !! destination object
...
!** FLOW
    ...
    TYPE IS (FvlStr)
        IF (ALLOCATED(SrcObj%cStr)) THEN
            DstObj%cStr = SrcObj%cStr
        ELSE
            ALLOCATE(tCharLen(0) :: DstObj%cStr)
        END IF
    CLASS DEFAULT
        CALL Handle_ErrLevel('FvlStr_CopyAssign', ModName, ErrSevere, 'Type of SrcObj is invalid.')
    END SELECT

    RETURN

END SUBROUTINE FvlStr_CopyAssign

!**************************************************************************************

SUBROUTINE FvlStr_Clone(SrcObj, DstObj)

!** PURPOSE OF DstObj SUBROUTINE:
    !^ To clone the FvlStr object.   This is a deferred
...
!** FLOW:
    ...
    ! create the FvlStr object
    ALLOCATE(DstObj, SOURCE=SrcObj)

    RETURN

END SUBROUTINE FvlStr_Clone
...
END SUBROUTINE FvlStr_Free

!******************************************************************************

FUNCTION FvlStr_GetTypeName(Obj) RESULT(Name)

!** PURPOSE OF THIS ROUTINE:
    !^ To get the name of the FvlStr type.
    !  This is a deferred procedure inherited from the *Assignable* type.
...
END FUNCTION FvlStr_GetTypeName
...
FUNCTION FvlStr_HashFunc(Key) RESULT(Code)
...
! FLOW
    ...
    IF (ALLOCATED(Key%cStr)) THEN
        Chr = Key%cStr(1:1)
        KeySize = C_SIZEOF(Chr)*LEN(Key%cStr)
        Code = HashFuncOpt(Key%cStr, KeySize, HashSeed, RemoveSign=TrueVal)
    ELSE
        Code = 0_kIndex
    END IF
