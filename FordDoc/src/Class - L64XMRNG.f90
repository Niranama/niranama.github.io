...
!** MODULE PARAMETERS:
    ...
    tLong, PARAMETER    :: M64    = ToLong(Z'D1342543DE82EF95')
    ! Jump constant {m'} for an advance of the 64-bit LCG by 2^32.
    ! Computed as: {m' = m^(2^32) (mod 2^64)}.
    tLong, PARAMETER    :: M64P   = ToLong(Z'8D23804C00000001')
    ...
    ! s = m' * s + c' * c
    tLong, PARAMETER    :: C64P   = ToLong(Z'16691C9700000000')
    ! size of LCG states
    tIndex,   PARAMETER :: LCG_STATE_SIZE  = 2_kIndex
    ! Parameters for algorithm flag
    tInteger, PARAMETER, PUBLIC :: L64X128Mix      = 1  !! flag for L64X128Mix PRNG
    tInteger, PARAMETER, PUBLIC :: L64X128StarStar = 2  !! flag for L64X128StarStar PRNG
    tInteger, PARAMETER, PUBLIC :: L64X256Mix      = 3  !! flag for L64X256Mix PRNG
    tInteger, PARAMETER, PUBLIC :: L64X1024Mix     = 4  !! flag for L64X1024Mix PRNG
...
! FLOW
    ...
    ! set states
    RNG%A = IOR(Seed0(0), 1_kLong)   ! Additive parameter must be odd
    RNG%S = Seed0(1)
...
! FLOW
    ...
    ! Compute the RandNum based on current state information
    ! (this allows the computation to be overlapped with state update).
    RandNum = RotateLeft((RNG%S + RNG%X(0))*5_kLong, 7)*9_kLong   ! "starstar" scrambler
