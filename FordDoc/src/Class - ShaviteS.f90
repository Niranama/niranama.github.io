...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: IV224_BE(0:7) = [            &
        ToInteger(Z'C4C67795'), ToInteger(Z'C0B1817F'), & 
        ToInteger(Z'EAD88924'), ToInteger(Z'1ABB1BB0'), &
        ToInteger(Z'E0C29152'), ToInteger(Z'BDE046BA'), & 
        ToInteger(Z'AEEECF99'), ToInteger(Z'58D509D8')]
    tInteger, PARAMETER :: IV224_LE(0:7) = [            &
        ToInteger(Z'6774F31C'), ToInteger(Z'990AE210'), & 
        ToInteger(Z'C87D4274'), ToInteger(Z'C9546371'), &
        ToInteger(Z'62B2AEA8'), ToInteger(Z'4B5801D8'), & 
        ToInteger(Z'1B702860'), ToInteger(Z'842F3017')]
    tInteger, PARAMETER :: IV256_BE(0:7) = [            &
        ToInteger(Z'3EECF551'), ToInteger(Z'BF10819B'), & 
        ToInteger(Z'E6DC8559'), ToInteger(Z'F3E23FD5'), &
        ToInteger(Z'431AEC73'), ToInteger(Z'79E3F731'), & 
        ToInteger(Z'98325F05'), ToInteger(Z'A92A31F1')]
    tInteger, PARAMETER :: IV256_LE(0:7) = [            &
        ToInteger(Z'49BB3E47'), ToInteger(Z'2674860D'), & 
        ToInteger(Z'A8B392AC'), ToInteger(Z'021AC4E6'), &
        ToInteger(Z'409283CF'), ToInteger(Z'620E5D86'), & 
        ToInteger(Z'6D929DCB'), ToInteger(Z'96CC2A8B')]
...
! FLOW
    ...
    BitLen = SHIFTL(MD%GetBlockCount() + 1_kLong, 9)
...
! FLOW
    ...
    ASSOCIATE(TmpBuf => MD%BufArr)
        ...
        ELSE
            ...
            Cnt1 = 0
        END IF
        CALL ByteUnpackLE(ToInteger(BitLen) + NBits, TmpBuf, 54_kIndex)
        CALL ByteUnpackLE(ToInteger(SHIFTR(BitLen, 32)), TmpBuf, 58_kIndex)
        DLen = MD%GetDigestLen()
        TmpBuf(62) = ToByte(SHIFTL(DLen, 3))
        TmpBuf(63) = ToByte(SHIFTR(DLen, 5))
        CALL MD%Process(MD%H, TmpBuf, Cnt0, Cnt1)
    END ASSOCIATE
