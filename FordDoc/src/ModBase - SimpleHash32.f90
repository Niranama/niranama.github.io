...
#undef  tHash
#define tHash           tInteger

!** MODULE PARAMETERS:
    ! module name
    tCharStar, PARAMETER    :: ModName = 'ModBase_SimpleHash32'
    ! kind of hash code
    tInteger,  PARAMETER    :: kHash = kInt32
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    tHash, PARAMETER    :: HighBits         = SHIFTL(ToInteger(Z'FFFFFFFF'), (HashBits-OneEighthBits))
...
!** FLOW
    ...
    ELSE
        GetKeyNum => Get_KeyNum_BE
    END IF
    SET_OPTION(HashCode, ToInteger(Z'AAAAAAAA'), StartHash)
    CALL AnyType_2_ByteArrPtr(Key, KeySize, KeyPtr)
...
!** FLOW
    ...
    DO I = 1, SIZE(KeyPtr, KIND=kIndex)
        ...
        HashCode = IEOR(SHIFTR(HashCode, 16), HashCode) * ToInteger(Z'45D9F3B')
        HashCode = IEOR(SHIFTR(HashCode, 16), HashCode) * ToInteger(Z'45D9F3B')
...
!** FLOW
    ...
    KeyNum = IAND(ToInteger(KeyByte), ToInteger(Z'000000FF'))

    ! alternative implementation
    !KeyNum = TRANSFER([KeyByte, 0_kByte, 0_kByte, 0_kByte], 0_kHash)
...
!** FLOW
    ...
    KeyNum = SHIFTL(IAND(ToInteger(KeyByte), ToInteger(Z'000000FF')), 24)
...
END MODULE ModBase_SimpleHash32
