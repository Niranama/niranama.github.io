...
#ifdef Indx64Bits
    USE ModBase_SimpleHash64,           ONLY: ComputeHash => Hash64_DJB
#else
    USE ModBase_SimpleHash32,           ONLY: ComputeHash => Hash32_DJB
#endif
    USE Class_Assignable
    USE Class_Comparable
    USE Class_IntrusiveBSTrees
    USE Class_IntrusiveHashTree
    USE Class_FvlStr
...
!** DERIVED TYPE DEFINITIONS
    ...
    CONTAINS
        ...
        PROCEDURE   :: CopyAssign       => CharIndex_Copy
        ...
        PROCEDURE   :: MakeClone        => CharIndex_Clone
        ...
        !  pointer component(s).
        PROCEDURE   :: FreeMemory       => CharIndex_Free
        ...
        PROCEDURE   :: GetTypeName      => CharIndex_GetTypeName
        ...
        PROCEDURE   :: CompareTo        => CharIndex_CompareKey
        !> *ComputeHashValue* is a procedure deferred by the *HashTreeNode* type. <br>
        !  Use the *HashCode* method in place of the *ComputeHashValue* method to
        !  compute hash code of the key component of the *HashTreeNode* object.
        PROCEDURE   :: ComputeHashValue => CharIndex_ComputeHashValue
        ! ---------------------------------------------------------------------
    END TYPE
...
SUBROUTINE CharIndex_Copy(DstObj, SrcObj)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To copy the CharIndex object.
    !  This is a deferred procedure by an *Assignable* object.

!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(CharIndex),  INTENT(OUT)  :: DstObj   !! destination object
...
! FLOW
    ...
    CLASS DEFAULT
        CALL Handle_ErrLevel('CharIndex_Copy', ModName, ErrSevere, &
                             'Type of the source object must be "CharIndex" only.')
        RETURN
    END SELECT
...
SUBROUTINE CharIndex_Clone(SrcObj, DstObj)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To clone the CharIndex object.
    !  This is a deferred procedure by an *Assignable* object.
...
! FLOW
    ...
    ! create the CharIndex object
    ALLOCATE(DstObj, MOLD=SrcObj)
    
    ! make a copy of the CharIndex object
    SELECT TYPE (DstObj)
    TYPE IS (CharIndex)
        DstObj = SrcObj
    END SELECT
...
END SUBROUTINE CharIndex_Clone
...
END SUBROUTINE CharIndex_Free

!******************************************************************************

FUNCTION CharIndex_GetTypeName(Obj) RESULT(Name)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To get the name of the CharIndex type.
    !  This is a deferred procedure by an *Assignable* object.
...
END FUNCTION CharIndex_GetTypeName

!******************************************************************************

FUNCTION CharIndex_CompareKey(A, B) RESULT(Flag)
...
! FLOW
    ...
    CLASS DEFAULT
        Flag = -999
        CALL Handle_ErrLevel('CharIndex_CompareKey', ModName, ErrSevere, 'Type of B is valid.')
    END SELECT

    RETURN

END FUNCTION CharIndex_CompareKey

!******************************************************************************

FUNCTION CharIndex_ComputeHashValue(Obj) RESULT(Code)
...
END FUNCTION CharIndex_ComputeHashValue
