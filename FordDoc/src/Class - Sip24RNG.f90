...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(LongRNG)  :: Sip24RNG
        PRIVATE
        !% initial state
        tLong   :: InitState(SEED_SIZE)
        !% current state
        tLong   :: State(SEED_SIZE)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tLong, PARAMETER    :: IV(4) = [ToLong(Z'736F6D6570736575'), &
                                    ToLong(Z'646F72616E646F6D'), &
                                    ToLong(Z'6C7967656E657261'), &
                                    ToLong(Z'7465646279746573')]
...
!** FLOW
    ...
    ASSOCIATE(V0 => RNG%State(1), V1 => RNG%State(2), &
        ...
        DoubleRound(V0, V1, V2, V3)
        V2 = IEOR(V2, ToLong(Z'00000000000000FF'))
        DoubleRound(V0, V1, V2, V3)
        DoubleRound(V0, V1, V2, V3)
    END ASSOCIATE
