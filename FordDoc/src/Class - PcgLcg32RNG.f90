...
! FLOW
    ...
    LngSeed(1) = IOR(IAND(ToLong(IntSeed(1)), MaskL), SHIFTL(ToLong(IntSeed(2)), 32)) &
                    + GOLDEN_RATIO_64
    LngSeed(2) = IOR(IAND(ToLong(IntSeed(3)), MaskL), SHIFTL(ToLong(IntSeed(4)), 32)) &
                    + GOLDEN_RATIO_64
...
! FLOW
    ...
    RNG%Increment = IOR(SHIFTL(Seed(2), 1), 1_kLong)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(PcgLcg32RNG), INTENT(IN)  :: RNG      !! 'PcgLcg32RNG' object
    tLong,              INTENT(IN)  :: Input    !! current state
    tLong                           :: Output   !! next state
...
! FLOW
    ...
    Output = Input * 6364136223846793005_kLong + RNG%Increment
...
! FLOW
    ...
    Count  = ToInteger(SHIFTR(Input, 59))
    Output = RotateRight(ToInteger(SHIFTR(IEOR(Input, SHIFTR(Input, 18)), 27)), Count)
...
! FLOW
    ...
    Count  = ToInteger(SHIFTR(Input, 61))
    Output = ToInteger(SHIFTR(IEOR(Input, SHIFTR(Input, 22)), 22 + Count))
