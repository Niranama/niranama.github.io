...
!   errors.  Unlike the <a href="../module/class_error_data.html#type-errordata">
...
!** USE STATEMENTS:
    USE ModBase_Common
    USE ModBase_DoublyLinkedLists,  ONLY: StackChar => ListCharacter
...
!** FLOW
    ...
    CALL ErrInstance%SubStack%Destruct()
    CALL ErrInstance%ModStack%Destruct()
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ErrorInfo),   INTENT(INOUT)   :: ErrInstance  !! 'ErrorInfo' object
    tInteger, OPTIONAL, INTENT(INOUT)   :: OutUnit      !! output unit
...
!** FLOW
    ...
    IF ((.NOT.ErrInstance%Handled).AND.(ErrInstance%Level > ErrNone)) THEN
        ...
        DO WHILE ((.NOT.ErrInstance%SubStack%IsEmpty()).AND. &
            ...
            Success = ErrInstance%SubStack%PeekTop(SubName)
            Success = ErrInstance%ModStack%PeekTop(ModName)
            I = I + 1
