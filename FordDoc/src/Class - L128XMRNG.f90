...
!** USE STATEMENTS:
    ...
    USE ModBase_UIntUtil,           ONLY: UnsignedMultiplyHigh => UMul128_Upper64
    USE Class_BaseRNG
    USE Class_LongRNG
...
!** MODULE PARAMETERS:
    ...
    tLong,    PARAMETER :: M128L  = ToLong(Z'D605BBB58C8ABBFD')
    ...
    tLong,    PARAMETER :: M128PH = ToLong(Z'31F179F5224754F4')
    ...
    ! s = m' * s + c' * c
    tLong,    PARAMETER :: C128PH = ToLong(Z'61139B28883277C3')
    ! size of LCG states
    tIndex,   PARAMETER :: LCG_STATE_SIZE = 4_kIndex
    ! Parameters for algorithm flag
    tInteger, PARAMETER, PUBLIC :: L128X128Mix  = 1 !! flag for L128X128Mix PRNG
    tInteger, PARAMETER, PUBLIC :: L128X256Mix  = 2 !! flag for L128X256Mix PRNG
    tInteger, PARAMETER, PUBLIC :: L128X1024Mix = 3 !! flag for L128X1024Mix PRNG
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(LongRNG)  :: L128XMRNG
        ...
        tInteger            :: Indx
        ! algorithm flag
        tInteger            :: L128XMAlgo = L128X128Mix
...
! FLOW
    ...
    ! set states
    RNG%AH = Seed0(0)
    RNG%AL = IOR(Seed0(1), 1_kLong)   ! Additive parameter must be odd
    RNG%SH = Seed0(2)
    RNG%SL = Seed0(3)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    tLong, INTENT(IN)   :: X, Y     !! Y is assumed to have the lowest bit set to 1.
    tLong               :: Z        !! the carry (either 0 or 1)
...
!** FLOW
    ...
    Z = SHIFTL((SHIFTR(X, 1) + SHIFTR(Y, 1) + IAND(X, 1_kLong)), 1)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger,         INTENT(OUT)   :: NextIndex    !! dummy here
    tLong                           :: RandNum      !! random number
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger,         INTENT(IN)    :: NextIndex    !! dummy here
