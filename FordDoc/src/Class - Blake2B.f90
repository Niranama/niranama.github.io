...
!   <a href="../module/class_blake2core.html#type-blake2core">Blake2Core</a>
...
! FLOW
    ...
    MD%T1 = 0_kLong
    MD%H(0:7) = IV(0:7)
    MD%H(0) = IEOR(MD%H(0), ToLong(IOR(IOR(MD%GetDigestLen(), SHIFTL(MD%GetKeyLen(), 8)), ToInteger(Z'01010000'))))
    CALL MD%CoreReset()
...
! FLOW
    ...
    IF ((MD%T0 == 0_kLong).AND.(MD%GetBufLen() > 0_kIndex)) MD%T1 = MD%T1 + 1_kLong
