...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(MDEngine) :: SHA3
        ...
        tByte       :: BufInp(0:143) = 0_kByte
        !% state
        tLong       :: State(0:24) = 0_kLong
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tLong, PARAMETER    :: MaxU64 = ToLong(Z'FFFFFFFFFFFFFFFF')
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    tByte, PARAMETER    :: Keccak_Suffix = ToByte(Z'01')
    tByte, PARAMETER    :: SHA3_Suffix   = ToByte(Z'06')
        ...
        BLOCK
            ...
            IF (Ptr == (BlockLen-1)) THEN
                IF (FinalNBits < 7) THEN
                    Buf(0) = IOR(ToByte(FinalInput), FByte80)
                    BufLen = 1
                ELSEIF (FinalNBits == 7) THEN
                    Buf(0) = ToByte(FinalInput)
                    Buf(1:BlockLen-1) = FByte00
                    Buf(BlockLen) = FByte80
                    BufLen = BlockLen + 1
                ELSE
                    Buf(0) = ToByte(FinalInput)
                    Buf(1) = ToByte(SHIFTR(FinalInput, 8))
            ...
            ELSE
                ...
                ELSE
                    IF (Ptr == (BlockLen-2)) THEN
                        Buf(0) = ToByte(FinalInput)
                        Buf(1) = IOR(ToByte(SHIFTR(FinalInput, 8)), FByte80)
                        BufLen = BlockLen - Ptr
                    ELSE
                        Buf(0) = ToByte(FinalInput)
                        Buf(1) = ToByte(SHIFTR(FinalInput, 8))
