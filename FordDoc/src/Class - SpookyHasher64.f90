...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(SpookyHasher64), TARGET, INTENT(INOUT)    :: HS       !! a hasher (HS) object
    tLong,                         INTENT(IN)       :: Seed1    !! seed1
    tLong,                         INTENT(IN)       :: Seed2    !! seed2
    tLogical,            OPTIONAL, INTENT(IN)       :: RemoveSign
    !^ flag indicating whether to remove sign bit or not. <br>
    !  - If true, always returns a positive value of the hash code. <br>
    !  - If false, returns either a positive or negative value of the hash code. <br>
...
!** FLOW
    ...
    ! remove sign if needed
    IF (HS%RemoveSign) HashCode%High = IAND(HashCode%High, ToLong(Z'7FFFFFFFFFFFFFFF'))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tLong, OPTIONAL, INTENT(OUT)    :: HashHigh     ! 64-bit upper hash code
    tLong                           :: HashCode     ! 64-bit lower hash code
...
END MODULE Class_SpookyHasher64
