...
!** MODULE PARAMETERS:
    ...
    ! -----------------------------------------------------------------------------------
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'Class_BaseRNG'
    ! +++ character sets +++
    ...
    tInteger,  PARAMETER    :: GOLDEN_RATIO_32 = ToInteger(Z'9E3779B9')
    ...
    !  Useful for producing good *Weyl* sequences or as an arbitrary nonzero odd value.
    tInteger,  PARAMETER    :: SILVER_RATIO_32 = ToInteger(Z'6A09E667')
    !% The first 64 bits of the silver ratio 1+sqrt(2), forced to be odd.
    !  Useful for producing good *Weyl* sequences or as an arbitrary nonzero odd value.
    tLong,     PARAMETER    :: SILVER_RATIO_64 = ToLong(Z'6A09E667F3BCC909')
    ! +++ parameters for character-string type +++
    tInteger,  PARAMETER    :: AlphaOnlyCap      = 1 !! upper-case alphabet
    tInteger,  PARAMETER    :: AlphaOnlyMix      = 2 !! mixed-case alphabet
    tInteger,  PARAMETER    :: AlphaNumericCap   = 3 !! upper-case alphabet + decimal number
    tInteger,  PARAMETER    :: AlphaNumericMix   = 4 !! mixed-case alphabet + decimal number
    tInteger,  PARAMETER    :: DecimalString     = 5 !! decimal (number) string
    tInteger,  PARAMETER    :: HexadecimalString = 6 !! hexadecimal (number) string
...
!** INTERFACE DEFINITIONS:
    ...
    ABSTRACT INTERFACE
        ...
        END FUNCTION
    END INTERFACE
    ! interface for procedures in the SubClass_Rng_Ziggurate submodule
    INTERFACE
        ...
        END FUNCTION
    END INTERFACE
    ! interface for procedures in the SubClass_Rng_Auxiliary submodule
    INTERFACE
...
! FLOW
    ...
    RandNum = ToByte(SHIFTR(RNG%NextInteger(), 24))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG              !! 'BaseRNG' object
    tByte,          INTENT(OUT)     :: ByteArray(0:)    !! random numbers
...
! FLOW
    ...
    RandNum = ToShort(SHIFTR(RNG%NextInteger(), 16))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG              !! 'BaseRNG' object
    tShort,         INTENT(OUT)     :: ShortArray(0:)   !! random numbers
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tLong, PARAMETER    :: POW_32 = SHIFTL(1_kLong, 32)
    tLong, PARAMETER    :: MaskL  = ToLong(Z'FFFFFFFFFFFFFFFF')

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tInteger    :: Lower, Diff, AddVal
    tLong       :: M, L, T
...
! FLOW
    ...
    M = IAND(ToLong(RNG%NextInteger()), MaskL) * Diff
    L = IAND(M, MaskL)
    IF (L < Diff) THEN
        ! 2^32 % N
        T = MOD(POW_32, ToLong(Diff))
        DO WHILE (L < T)
            M = IAND(ToLong(RNG%NextInteger()), MaskL) * ToLong(Diff)
            L = IAND(M, MaskL)
        END DO
    END IF
    AddVal = ToInteger(SHIFTR(M, 32))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG              !! 'BaseRNG' object
    tInteger,       INTENT(OUT)     :: IntegerArray(:)  !! random numbers
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG),  INTENT(INOUT)  :: RNG      !! 'BaseRNG' object
    tLong,           INTENT(IN)     :: Bound1   !! a required limit
    tLong, OPTIONAL, INTENT(IN)     :: Bound2   !! an optional limit
...
! FLOW
    ...
    ELSE
        Diff  = ABS(Bound1)
        Lower = MIN(Bound1, 0_kLong)
    END IF

    ! return quickly if Diff is zero
    IF (Diff == 0_kLong) THEN
        RandNum = Bound1
        RETURN
    END IF
    ...
    DO
        ...
        IF (Bits - AddVal + (Diff - 1_kLong) >= 0_kLong) EXIT
    END DO
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG          !! 'BaseRNG' object
    tLong,          INTENT(OUT)     :: LongArray(:) !! random numbers
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG      !! 'BaseRNG' object
    tSingle                         :: RandNum  !! random number

!** SUBROUTINE PARAMETER DECLARATIONS:
    ! although these three parameters use different formulas, they are essentially the same.
    tSingle, PARAMETER  :: SNorm1 = 2.0_kSingle**(-24)
    tSingle, PARAMETER  :: SNorm2 = 1.0_kSingle/SHIFTL(1, 24)
    tSingle, PARAMETER  :: SNorm3 = 0.5_kSingle*EPSILON(1.0_kSingle)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG),    INTENT(INOUT)    :: RNG      !! 'BaseRNG' object
    tSingle,           INTENT(IN)       :: Bound1   !! a required limit
    tSingle, OPTIONAL, INTENT(IN)       :: Bound2   !! an optional limit
    tSingle                             :: RandNum  !! random number
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG              !! 'BaseRNG' object
    tSingle,        INTENT(OUT)     :: SingleArray(:)   !! random numbers
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG),    INTENT(INOUT)    :: RNG      !! 'BaseRNG' object
    tDouble,           INTENT(IN)       :: Bound1   !! a required limit
    tDouble, OPTIONAL, INTENT(IN)       :: Bound2   !! an optional limit
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG              !! 'BaseRNG' object
    tDouble,        INTENT(OUT)     :: DoubleArray(:)   !! random numbers
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG),  INTENT(INOUT)  :: RNG      !! 'BaseRNG' object
    tQuad,           INTENT(IN)     :: Bound1   !! a required limit
    tQuad, OPTIONAL, INTENT(IN)     :: Bound2   !! an optional limit
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG), INTENT(INOUT)   :: RNG              !! 'BaseRNG' object
    tQuad,          INTENT(OUT)     :: QuadArray(:)     !! random numbers
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(BaseRNG),     INTENT(INOUT)   :: RNG      !! 'BaseRNG' object
    tInteger, OPTIONAL, INTENT(IN)      :: StrType  !! type of string (1-6)
    tInteger, OPTIONAL, INTENT(IN)      :: StrLen   !! length of output string
    tInteger, OPTIONAL, INTENT(IN)      :: MaxLen   !! maximum length of output string; if StrLen present, this parameter is ignored
    tCharAlloc                          :: RandStr  !! random string

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tInteger    :: LenMax, OutLen, I, J
