...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: CS(0:15) = [                     &
            ToInteger(Z'243F6A88'), ToInteger(Z'85A308D3'), &
            ToInteger(Z'13198A2E'), ToInteger(Z'03707344'), &
            ToInteger(Z'A4093822'), ToInteger(Z'299F31D0'), &
            ToInteger(Z'082EFA98'), ToInteger(Z'EC4E6C89'), &
            ToInteger(Z'452821E6'), ToInteger(Z'38D01377'), &
            ToInteger(Z'BE5466CF'), ToInteger(Z'34E90C6C'), &
            ToInteger(Z'C0AC29B7'), ToInteger(Z'C97C50DD'), &
            ToInteger(Z'3F84D5B5'), ToInteger(Z'B5470917')]
...
! FLOW
    ...
    ! initialize
    MD%T(0) = MD%T(0) + 512_kInteger
    IF (IAND(MD%T(0), NOT(ToInteger(Z'1FF'))) == 0_kInteger) MD%T(1) = MD%T(1) + 1
    V(0:3) = MD%S(0:3)
    V(4:5) = MD%T(0)
    V(6:7) = MD%T(1)
    DO I = 0, 7
...
! FLOW
    ...
    ASSOCIATE(TmpBuf => MD%BufArr)   
        ...
        IF ((Ptr == 0_kIndex).AND.(NBits == 0_kIndex)) THEN
            MD%T(0) = ToInteger(Z'FFFFFE00')
            MD%T(1) = ToInteger(Z'FFFFFFFF')
        ELSEIF (MD%T(0) == 0) THEN
            MD%T(0) = ToInteger(Z'FFFFFE00') + BitLen
            MD%T(1) = MD%T(1) - 1_kInteger
        ELSE
            MD%T(0) = MD%T(0) - (512_kInteger - BitLen)
        END IF
        IF (BitLen <= 446_kIndex) THEN
            TmpBuf(Ptr+1:55) = 0_kByte
            IF (MD%GetDigestLen() == 32_kIndex) TmpBuf(55) = IOR(TmpBuf(55), 1_kByte)
            CALL ByteUnpackBE(TH, TmpBuf, 56_kIndex)
            CALL ByteUnpackBE(TL, TmpBuf, 60_kIndex)
            CALL MD%Update(TmpBuf, Ptr, 64_kIndex - Ptr)
        ELSE
            TmpBuf(Ptr+1:63) = 0_kByte
            CALL MD%Update(TmpBuf, Ptr, 64_kIndex - Ptr)
            MD%T(0) = ToInteger(Z'FFFFFE00')
            MD%T(1) = ToInteger(Z'FFFFFFFF')
            TmpBuf(0:55) = 0_kByte
            IF (MD%GetDigestLen() == 32_kIndex) TmpBuf(55) = 1_kByte
            CALL ByteUnpackBE(TH, TmpBuf, 56_kIndex)
            CALL ByteUnpackBE(TL, TmpBuf, 60_kIndex)
