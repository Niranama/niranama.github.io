...
!** MODULE PARAMETERS:
    ...
    tLong,    PARAMETER :: INT_MASK_LONG   = ToLong(Z'00000000FFFFFFFF')
    ! Most significant w-r bits.
    tLong,    PARAMETER :: UPPER_MASK_LONG = ToLong(Z'0000000080000000')
    ! Least significant r bits.
    tLong,    PARAMETER :: LOWER_MASK_LONG = ToLong(Z'000000007FFFFFFF')
    ! Most significant w-r bits.
    tInteger, PARAMETER :: UPPER_MASK = ToInteger(Z'80000000')
    ! Least significant r bits.
    tInteger, PARAMETER :: LOWER_MASK = ToInteger(Z'7FFFFFFF')
    ! Size of the bytes pool.
    tInteger, PARAMETER :: N = 624
    ! Period second parameter.
    tInteger, PARAMETER :: M = 397
    ! X * MATRIX_A for X = {0, 1}.
    tInteger, PARAMETER :: MAG01(0:1) = [0, ToInteger(Z'9908B0DF')]
...
! FLOW
    ...
    ! FLOW
        ...
        State(0) = ToInteger(UPPER_MASK_LONG)
    ...
    ! FLOW
        ...
        MT = IAND(19650218_kLong, INT_MASK_LONG)
        State(0) = ToInteger(MT)
        DO I = 1, SIZE(State)-1
            MT = IAND((1812433253_kLong * IEOR(MT, SHIFTA(MT, 30)) + I), INT_MASK_LONG)
            State(I) = ToInteger(MT)
        END DO
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        tInteger, INTENT(INOUT) :: State(0:)
        tInteger, INTENT(IN)    :: Seed(0:)
        tInteger                :: NextID

    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        tInteger    :: StateSize, SeedSize, MaxSize
        tInteger    :: I, J, K
        tLong       :: A, B, C
    ...
    ! FLOW
        ...
        DO K = MaxSize, 1, -1
            IF (State(I) < 0) THEN
                A = IOR(IAND(ToLong(State(I)), LOWER_MASK_LONG), UPPER_MASK_LONG)
            ELSE
                A = IOR(IAND(ToLong(State(I)), LOWER_MASK_LONG), 0_kLong)
            END IF
            IF (State(I-1) < 0) THEN
                B = IOR(IAND(ToLong(State(I-1)), LOWER_MASK_LONG), UPPER_MASK_LONG)
            ELSE
                B = IOR(IAND(ToLong(State(I-1)), LOWER_MASK_LONG), 0_kLong)
            END IF
            ! Non linear
            C = IEOR(A, (IEOR(B, SHIFTA(B, 30)) * 1664525_kLong)) + Seed(J) + J
            State(I) = ToInteger(IAND(C, INT_MASK_LONG))
            I = I + 1
            J = J + 1
            IF (I >= StateSize) THEN
    ...
    ! FLOW
        ...
        DO K = StateSize, 1, -1
            ...
            END IF
            ! Non linear
            C = IEOR(A, (IEOR(B, SHIFTA(B, 30)) * 1566083941_kLong)) - I
            State(I) = ToInteger(IAND(C, INT_MASK_LONG))
            I = I + 1
            IF (I >= StateSize) THEN
                State(0) = State(StateSize - 1)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    tInteger, PARAMETER :: TMaskB = ToInteger(Z'9D2C5680')
    tInteger, PARAMETER :: TMaskC = ToInteger(Z'EFC60000')

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tInteger    :: Y, MTCurr, MTNext, K
