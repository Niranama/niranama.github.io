...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'Class_Lfsr258RNG'
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(LongRNG)  :: Lfsr258RNG
        PRIVATE
        tLong       :: State1 =  153587801
        tLong       :: State2 = -759022222
        tLong       :: State3 =  1288503317
        tLong       :: State4 = -1718083407
        tLong       :: State5 = -123456789
    CONTAINS
        ! ---------------------------------------------------------------------
        ! -----                 Deferred Procedures                       -----
        ! ---------------------------------------------------------------------
...
! FLOW
    ...
    IF (IAND(RNG%State1,       -2_kLong) == 0_kLong) RNG%State1 = RNG%State1 - 8388607_kLong
    IF (IAND(RNG%State2,     -512_kLong) == 0_kLong) RNG%State2 = RNG%State2 - 8388607_kLong
    IF (IAND(RNG%State3,    -4096_kLong) == 0_kLong) RNG%State3 = RNG%State3 - 8388607_kLong
    IF (IAND(RNG%State4,  -131072_kLong) == 0_kLong) RNG%State4 = RNG%State4 - 8388607_kLong
    IF (IAND(RNG%State5, -8388608_kLong) == 0_kLong) RNG%State5 = RNG%State5 - 8388607_kLong
...
! FLOW
    ...
    RNG%State1 = IEOR(SHIFTL(IAND(RNG%State1,       -2_kLong), 10), B)
    B          = SHIFTR(IEOR(SHIFTL(RNG%State2,     24), RNG%State2), 50)
    RNG%State2 = IEOR(SHIFTL(IAND(RNG%State2,     -512_kLong), 5), B)
    B          = SHIFTR(IEOR(SHIFTL(RNG%State3,      3), RNG%State3), 23)
    RNG%State3 = IEOR(SHIFTL(IAND(RNG%State3,    -4096_kLong), 29), B)
    B          = SHIFTR(IEOR(SHIFTL(RNG%State4,      5), RNG%State4), 24)
    RNG%State4 = IEOR(SHIFTL(IAND(RNG%State4,  -131072_kLong), 23), B)
    B          = SHIFTR(IEOR(SHIFTL(RNG%State5,      3), RNG%State5), 33)
    RNG%State5 = IEOR(SHIFTL(IAND(RNG%State5, -8388608_kLong), 8), B)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    ! equivalent a NORM = 1.0 / ToLong(Z'FFFFFFFFFFFFF800')
    tDouble, PARAMETER  :: DNorm = 0.5_kDouble/ToLong(Z'7FFFFFFFFFFFFC00')
    tDouble, PARAMETER  :: DMax  = ToLong(Z'FFFFFFFFFFFFF800')*DNorm + 1.0_kDouble
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tLong,   PARAMETER  :: TwoPow66 = ToLong(Z'4000000000000000')   ! 4*(2**64)
