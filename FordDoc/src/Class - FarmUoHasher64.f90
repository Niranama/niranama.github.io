...
!** FLOW
    ...
    ASSOCIATE ( X => HS%State(1),  Y => HS%State(2),  Z => HS%State(3), &
        ...
        U = X - Z
        X = X*K2
        M = K2 + IAND(U, ToLong(Z'0000000000000082'))
    END ASSOCIATE
    ...
    IF (HS%GetBlockCount() > 0_kIndex) THEN
        ...
        BLOCK
            ...
            ASSOCIATE ( X => HS%State(1),  Y => HS%State(2),  Z => HS%State(3), &
                ...
                W0 = W0 + ToLong(IAND(Length - 1_kIndex, 63_kIndex))
                U = U + Y
                Y = Y + U
...
END MODULE Class_FarmUoHasher64
