...
MODULE Class_HTabHashable

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *HTabHashable* type, and its related routines.   The
!   *HTabHashable* type is a container type that employs an open-addressing hash table
!   implementation to provide common operations for an unordered symbol table.  As a
!   symbol table, the *HTabHashable* type uses the *Hashable* derived type to store the
!   keys and their associated values.  Unlike other hash-table containers, the
!  *HTabHashable* type uses a user-defined type in the *Hashable* class to represent
...
!   concrete* subtype of the *Hashable* type.  The *HTabHashable* type employs the
...
!   It is also worth mentioning that the *HTabHashable* type does not allow key-value objects
!   of different types to be stored in the same container.  The *HTabHashable* type requires
...
!   objects into a container, the *HTabHashable* type requires an explicit construction before
...
!   As a symbol table, the *HTabHashable* type does not allow duplicated keys.  Therefore, if
...
!   Technically, the *HTabHashable* type employs the open-addressing as a collision resolution
...
#define     tHash           tIndex
!----------------------------------------------------------
#define     HashTable       HTabHashable
#define     QueueKeyVal     ListAssignable
!----------------------------------------------------------
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'Class_HTabHashable'
    ! default capacity
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(Hashable),    INTENT(IN)      :: NulObj
    ...
    CLASS(Hashable),    INTENT(IN)      :: DelObj
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(Hashable),    INTENT(IN)      :: KeyVals(:)
    !> a user-defined object representing a null object; the user should never specify this
    !  object in any operations other than the construction operations.
    CLASS(Hashable),    INTENT(IN)      :: NulObj
    !> a user-defined object representing a deleted object (different from the null object); the user
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    CLASS(Hashable), ALLOCATABLE    :: OldBuckets(:)
    tIndex                          :: OldCap, I
...
!** FLOW:
    ...
    DO I = 0_kIndex, OldCap-1_kIndex
        IF ((OldBuckets(I) /= Table%NULOBJ).AND.(OldBuckets(I) /= Table%DELOBJ)) THEN
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(HashTable), INTENT(INOUT) :: Table    !! HashTable object
    CLASS(Hashable),  INTENT(IN)    :: KeyVal   !! key to be inserted and its associated value
...
! FLOW
    ...
    ! check and return quickly if needed
    IF ((KeyVal == Table%NULOBJ).OR.(KeyVal == Table%DELOBJ)) THEN
    ...
    Indx = 1_kIndex
    DO
        IF (Table%Buckets(I) == Table%DELOBJ) THEN
            ! The current bucket was previously deleted
            IF (J == -1_kIndex) J = I
        ELSEIF (Table%Buckets(I) /= Table%NULOBJ) THEN
            ! The current bucket already contains a key
            IF (Table%Buckets(I) == KeyVal) THEN
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(HashTable), INTENT(INOUT) :: Table    !! HashTable object
    CLASS(Hashable),  INTENT(IN)    :: KeyVal   !! key to be removed and its associated value
...
! FLOW
    ...
    DO
        ...
        ELSEIF (Table%Buckets(I) == Table%NULOBJ) THEN
            ...
            Flag = FalseVal
            EXIT
        ELSEIF (Table%Buckets(I) == KeyVal) THEN
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(HashTable), INTENT(INOUT) :: Table    !! HashTable object
    CLASS(Hashable),  INTENT(IN)    :: Key      !! key to be looked for
...
! FLOW
    ...
    ! check and return quickly if needed
    IF ((Key == Table%NULOBJ).OR.(Key == Table%DELOBJ)) THEN
        Found = FalseVal
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(HashTable), INTENT(INOUT) :: Table    !! HashTable object
    CLASS(Hashable),  INTENT(IN)    :: KeyVal   !! key to be looked for and its associated value
    !% flag indicating whether the specified key is found or not.
    tLogical                        :: Found
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(Hashable), OPTIONAL, INTENT(OUT)      :: KeyVal
...
! FLOW
    ...
    DO WHILE ((Table%Buckets(Table%Indx) == Table%NULOBJ).OR.(Table%Buckets(Table%Indx) == Table%DELOBJ))
...
! FLOW
    ...
    DO I = 0_kIndex, Table%Capacity-1_kIndex
        IF ((Table%Buckets(I) /= Table%DELOBJ).AND.(Table%Buckets(I) /= Table%NULOBJ)) THEN
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(HashTable), INTENT(INOUT) :: Table    !! HashTable object
    CLASS(Hashable),  INTENT(IN)    :: Key      !! key
