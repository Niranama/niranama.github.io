...
!   from the <a href="../module/class_basedigest.html#type-basedigest">
...
!   <a href="../module/class_mdengine.html#type-mdengine">MDEngine</a> type. <br>
...
!   The *KP1600Core* type utilizes the <a href="../module/class_kp1600sponge.html#type-kp1600sponge">
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, ABSTRACT, EXTENDS(BaseDigest) :: KP1600Core
        ...
        tByte               :: InitSuffix = ToByte(Z'80')
        !% The current bits appended to the end of input
        tByte               :: CurrSuffix = ToByte(Z'80')
    CONTAINS
        ! ---------------------------------------------------------------------
...
! FLOW
    ...
    IF (NBits > 0_kByte) THEN
        ...
        DelimitedLastBytes = IEOR(ToInteger(SHIFTR(LastByte, (8_kIndex - NBits))), &
                                  SHIFTL(ToInteger(MD%CurrSuffix), NBits))
        IF (IAND(DelimitedLastBytes, ToInteger(Z'0000FF00')) == 0_kInteger) THEN
            MD%CurrSuffix = ToByte(DelimitedLastBytes)
        ELSE
            OneByte(0) = ToByte(DelimitedLastBytes)
            Flag = MD%Sponge%Absorb(OneByte, 1_kIndex)
            MD%CurrSuffix = ToByte(SHIFTR(DelimitedLastBytes, 8))
        END IF
    END IF
