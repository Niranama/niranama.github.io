...
SUBROUTINE Generic_MemAlloc_1D(CharLen, Array, Size, StartID)
...
!** FLOW:
    ...
    IF (PRESENT(StartID)) THEN
        ALLOCATE(tCharLen(CharLen) :: Array(StartID:StartID+Size-1), STAT=AllocStat, ERRMSG=AllocMsg)
    ELSE
        ALLOCATE(tCharLen(CharLen) :: Array(Size), STAT=AllocStat, ERRMSG=AllocMsg)
    END IF
    ! handle error if any
...
SUBROUTINE Generic_MemAlloc_2D(CharLen, Array, Size1, Size2, StartID1, StartID2)
...
!** FLOW:
    ...
    CALL MemFree(Array)
    ! allocate memory
    ALLOCATE(tCharLen(CharLen) :: Array(BeginID1:EndID1, BeginID2:EndID2), STAT=AllocStat, ERRMSG=AllocMsg)
    ! handle error if any
...
SUBROUTINE Generic_MemAlloc_3D(CharLen, Array, Size1, Size2, Size3, StartID1, StartID2, StartID3)
...
!** FLOW:
    ...
    CALL MemFree(Array)
    ! allocate memory
    ALLOCATE(tCharLen(CharLen) :: Array(BeginID1:EndID1, BeginID2:EndID2, BeginID3:EndID3), STAT=AllocStat, ERRMSG=AllocMsg)
    ! handle error if any
