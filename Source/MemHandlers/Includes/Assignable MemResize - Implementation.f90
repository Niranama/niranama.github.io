...
!** FLOW:
    ...
    IF (OldSize == 0) THEN
        ...
        CALL MemAlloc(Array, NewSize, Array(0), StartID=StartID)
    ELSE
        ! first, allocate the temporary array
        CALL MemAlloc(Temp, NewSize, Array(1), StartID=StartID)
...
!** FLOW:
    ...
    IF ((OldSize1 == 0).OR.(OldSize2 == 0)) THEN
        ! just allocate the array
        CALL MemAlloc(Array, NewSize1, NewSize2, Array(0,0), StartID1=StartID1, StartID2=StartID2)
    ELSE
        ! first, allocate the temporary array
        CALL MemAlloc(Temp, NewSize1, NewSize2, Array(1,1), StartID1, StartID2)
...
!** FLOW:
    ...
    IF ((OldSize1 == 0).OR.(OldSize2 == 0).OR.(OldSize3 == 0)) THEN
        ! just allocate the array
        CALL MemAlloc(Temp, NewSize1, NewSize2, NewSize3, Array(0,0,0), StartID1, StartID2, StartID3)
    ELSE
        ! first, allocate the temporary array
        CALL MemAlloc(Temp, NewSize1, NewSize2, NewSize3, Array(1,1,1), StartID1, StartID2, StartID3)
        ! determine the preserving sizes
        IF (NewSize1 >= OldSize1) THEN
            PSize1 = OldSize1
        ELSE
