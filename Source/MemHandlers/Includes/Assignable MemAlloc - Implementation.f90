...
SUBROUTINE Generic_MemAlloc_1D(Array, Size, Mold, StartID)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    TypeName,                INTENT(IN)     :: Mold         !! mold (concrete type)
...
!** FLOW:
    ...
    IF (PRESENT(StartID)) THEN
        ALLOCATE(Array(StartID:StartID+Size-1), STAT=AllocStat, ERRMSG=AllocMsg, MOLD=Mold)
    ELSE
        ALLOCATE(Array(Size), STAT=AllocStat, ERRMSG=AllocMsg, MOLD=Mold)
    END IF
    ! handle error if any
...
SUBROUTINE Generic_MemAlloc_2D(Array, Size1, Size2, Mold, StartID1, StartID2)
...
!** FLOW:
    ...
    CALL MemFree(Array)
    ! allocate memory
    ALLOCATE(Array(BeginID1:EndID1, BeginID2:EndID2), STAT=AllocStat, ERRMSG=AllocMsg, MOLD=Mold)
    ! handle error if any
...
SUBROUTINE Generic_MemAlloc_3D(Array, Size1, Size2, Size3, Mold, StartID1, StartID2, StartID3)
...
!** FLOW:
    ...
    CALL MemFree(Array)
    ! allocate memory
    ALLOCATE(Array(BeginID1:EndID1, BeginID2:EndID2, BeginID3:EndID3), &
             STAT=AllocStat, ERRMSG=AllocMsg, MOLD=Mold)
    ! handle error if any
