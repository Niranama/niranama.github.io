...
!** FLOW:
    ...
    IF (OldSize == 0) THEN
        ...
        CALL MemAlloc(CharLen, Array, NewSize, StartID)
    ELSE
        ! first, allocate the temporary array
        CALL MemAlloc(CharLen, Temp, NewSize, StartID)
...
!** FLOW:
    ...
    IF ((OldSize1 == 0).OR.(OldSize2 == 0)) THEN
        ! just allocate the array
        CALL MemAlloc(CharLen, Array, NewSize1, NewSize2, StartID1, StartID2)
    ELSE
        ! first, allocate the temporary array
        CALL MemAlloc(CharLen, Temp, NewSize1, NewSize2, StartID1, StartID2)
...
!** FLOW:
    ...
    IF ((OldSize1 == 0).OR.(OldSize2 == 0).OR.(OldSize3 == 0)) THEN
        ! just allocate the array
        CALL MemAlloc(CharLen, Array, NewSize1, NewSize2, NewSize3, StartID1, StartID2, StartID3)
    ELSE
        ! first, allocate the temporary array
        CALL MemAlloc(CharLen, Temp, NewSize1, NewSize2, NewSize3, StartID1, StartID2, StartID3)
        ! determine the preserving sizes
        IF (NewSize1 >= OldSize1) THEN
            PSize1 = OldSize1
        ELSE
