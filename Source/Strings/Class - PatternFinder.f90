...
!** USE STATEMENTS:
    ...
    USE ModBase_Common
    USE ModBase_CharUtil,       ONLY: IsLetter => Is_Character_Letter, &
                                      ToUpper  => ToUpperCase, &
                                      ToLower  => ToLowerCase
...
!** MODULE PARAMETERS:
    ...
    tInteger, PUBLIC, PARAMETER :: BruteForce = 0
    tInteger, PUBLIC, PARAMETER :: BoyerMoore = 1
    tInteger, PUBLIC, PARAMETER :: KMPlus     = 2
    tInteger, PUBLIC, PARAMETER :: KMP        = 3
    tInteger, PUBLIC, PARAMETER :: RabinKarp  = 4
    tInteger, PUBLIC, PARAMETER :: Horspool   = 5
    tInteger, PUBLIC, PARAMETER :: Raita      = 6
    tInteger, PUBLIC, PARAMETER :: Sunday     = 7
    tInteger, PUBLIC, PARAMETER :: BNDM       = 8
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(BasePattern) :: RabinKarpPat
        ...
        tLong       :: Q        ! a large prime, small enough to avoid long overflow
        tIndex      :: R        ! radix
        tLong       :: RM       ! MOD(R**(M-1), Q)
    ...
    TYPE, EXTENDS(BasePattern) :: BNDMPatLen64
        tIndex             :: Lowest   ! ASCII code of lowest character
        tLong              :: DefVal   ! default value
        tLong, ALLOCATABLE :: Arr(:)   ! skip array
    CONTAINS
...
! FLOW
    ...
    DO I = 0_kIndex, N-1_kIndex
        IF (Pattern(I) == WC) THEN
            J = IOR(J, SHIFTL(1_kLong, N-I-1_kIndex))
        END IF
    END DO
...
! FLOW
    ...
    DO I = 0_kIndex, N-1_kIndex
        IF (Pattern(I) == WC) THEN
            J = IOR(J, SHIFTL(OneI128, N-I-1_kIndex))
        END IF
    END DO
    IF (.NOT.IsCaseInsensitive) THEN
        ! ----- case-sensitive -----
...
! FLOW
    ...
    CALL RNG%Initialize()
    RandVal = RNG%NextLong(MAX_I32+1_kLong, MAX_I64-MAX_I32)
    SELECT TYPE (Pat => PatObj)
    TYPE IS (RabinKarpPat)
        Pat%M = SIZE(Pattern, KIND=kIndex)
        Pat%R = Radix
...
CONTAINS
    ...
    FUNCTION LongRandomPrime(RndVal) RESULT(PrmVal)
        ...
        tLong, INTENT(IN)   :: RndVal
        tLong               :: PrmVal
        ! execution
        PrmVal = RndVal
        DO WHILE (.NOT.IS_PRIME(PrmVal))
            PrmVal = PrmVal + 1_kLong
        END DO
        RETURN
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tChar,               INTENT(IN)     :: Pattern(0:)  !! the pattern as an array of characters
    tIndex,              INTENT(IN)     :: EndID        !! ending index
    tLong,               INTENT(IN)     :: DefVal       !! default value
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tChar,               INTENT(IN)     :: C        !! the character
    tLong,               INTENT(IN)     :: Val      !! the new value
...
END MODULE Class_PatternFinder
