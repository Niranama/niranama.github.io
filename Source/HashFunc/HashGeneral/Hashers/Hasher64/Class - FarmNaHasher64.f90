...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(Hasher64) :: FarmNaHasher64
        PRIVATE
        !% seeds
        tLong       :: Seed(2)              = 0_kLong
        !% state
        tLong       :: State(7)             = 0_kLong
    ...
    IF (HS%GetBlockCount() > 0_kIndex) THEN
        ...
        BLOCK
            ...
            ASSOCIATE (X => HS%State(1), Y => HS%State(2), Z => HS%State(3), &
                       V1 => HS%State(4), V2 => HS%State(5), &
                       W1 => HS%State(6), W2 => HS%State(7))
                M = K1 + SHIFTL(IAND(Z, ToLong(Z'00000000000000FF')), 1)
                ...
                X = IEOR(X, W2*9_kLong)
                Y = Y + V1*9_kLong + PackLong(LastBuf, 40_kIndex)
...
END MODULE Class_FarmNaHasher64
