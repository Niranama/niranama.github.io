...
MODULE Class_HalfSip24Hasher32
...
!** MODULE PARAMETERS:
    tByte, PARAMETER    :: DefaultKey(8) = [ToByte(Z'AF'), ToByte(Z'D7'), &
                                            ToByte(Z'FB'), ToByte(Z'CA'), &
                                            ToByte(Z'BB'), ToByte(Z'4B'), &
                                            ToByte(Z'40'), ToByte(Z'7E')]
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(Hasher32) :: HalfSip24Hasher32
        PRIVATE
        !% state
        tInteger    :: State(4)    = 0_kInteger
        !% buffer array used to store input data
        tByte       :: BufArr(0:3) = 0_kByte
...
CONTAINS
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        tByte,  INTENT(INOUT)   :: Buf(0:)  ! buffer
...
END MODULE Class_HalfSip24Hasher32
