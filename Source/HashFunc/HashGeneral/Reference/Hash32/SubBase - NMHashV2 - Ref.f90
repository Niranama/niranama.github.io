...
SUBMODULE (ModBase_ReferenceHash32) SubBase_NMHashV2_Ref

!^ **PURPOSE OF THIS SUBMODULE**: <br>
!   This submodule contains a reference implementation for version 2 of the NM hash algorithms
!   by James Z. M. Gao. <br>
...
!** FLOW
    ...
    ! perform hashing
    HashCode = NM_V2_Hash32(InpPtr, Seed, ByteConv)
...
!** FLOW
    ...
    ! perform hashing
    HashCode = NMx_V2_Hash32(InpPtr, Seed, ByteConv)
...
FUNCTION NM_V2_Hash32(Input, Seed, BC) RESULT(HashCode)
...
#define NMHASH32_33to255(P, Len, Seed)  NMHASH32_9to255(P, Len, Seed, TrueVal)
    ...
    IF (Length <= 32) THEN
        ...
        ELSEIF (Length > 4) THEN
            BLOCK
                tInteger    :: X, Y
                X = BC%Pack_I32(Input, 0)
                Y = IEOR(BC%Pack_I32(Input, Length - 4), NMH_PRIME32_4 + 2 + Seed)
                X = X + Y
        ...
        ELSE
            BLOCK
                ...
                CASE(1:3)
                    ...
                    X = BC%Pack_I32_Partial(Input, 0, Length)
                CASE(4)
...
CONTAINS
    ...
    !** FLOW
        ...
        IF (FullAvalanche) THEN
            ...
            DO I = 0, R-1
                DO J = 0, 3
                    XU32(J) = IEOR(XU32(J), BC%Pack_I32(P, I*32 + J*4))
                    YU32(J) = IEOR(YU32(J), BC%Pack_I32(P, I*32 + J*4 + 16))
            ...
            END DO
            DO J = 0, 3
                XU32(J) = IEOR(XU32(J), BC%Pack_I32(P, Length - 32 + J*4))
                YU32(J) = IEOR(YU32(J), BC%Pack_I32(P, Length - 16 + J*4))
            END DO
        ELSE
            ! 9 to 32 bytes
            ShiftLen = SHIFTL(SHIFTA(Length, 4), 3)
            XU32(0) = IEOR(XU32(0), BC%Pack_I32(P, 0))
            XU32(1) = IEOR(XU32(1), BC%Pack_I32(P, ShiftLen))
            XU32(2) = IEOR(XU32(2), BC%Pack_I32(P, Length - 8))
            XU32(3) = IEOR(XU32(3), BC%Pack_I32(P, Length - 8 - ShiftLen))
            YU32(0) = IEOR(YU32(0), BC%Pack_I32(P, 4))
            YU32(1) = IEOR(YU32(1), BC%Pack_I32(P, ShiftLen + 4))
            YU32(2) = IEOR(YU32(2), BC%Pack_I32(P, Length - 8 + 4))
            YU32(3) = IEOR(YU32(3), BC%Pack_I32(P, Length - 8 - ShiftLen + 4))
        END IF
    ...
    !** FLOW
        ...
        DO I = 0, ACC_SIZE-1
            AccX(I) = IEOR(AccX(I), BC%Pack_I32(P, Offset))
            AccY(I) = IEOR(AccY(I), BC%Pack_I32(P, Offset + SizeOfACC))
...
FUNCTION NMx_V2_Hash32(Input, Seed, BC) RESULT(HashCode)
...
CONTAINS
    ...
    !** FLOW
        ...
        ! initialize
        X = IEOR(BC%Pack_I32(P, 0), NMH_PRIME32_3)
        Y = IEOR(BC%Pack_I32(P, Length - 4), Seed)
    ...
    !** FLOW
        ...
        DO I = 0, R-1
            X = IEOR(X, BC%Pack_I32(P, Offset + 0))
            ...
            A = IEOR(A, BC%Pack_I32(P, Offset + 8))
            B = IEOR(B, BC%Pack_I32(P, Offset + 12))
            A = IEOR(A, B)
            A = A*M1
        ...
        IF (IAND(ToInteger(Length)-1, 8) /= 0) THEN
            IF (IAND(ToInteger(Length)-1, 4) /= 0) THEN
                A = IEOR(A, BC%Pack_I32(P, Offset))
                B = IEOR(B, BC%Pack_I32(P, Offset + 4))
                A = IEOR(A, B)
                A = A*M1
                A = IEOR(A, SHIFTR(A, 23))
                A = A*M2
                ...
                A = A*M3
            ELSE
                A = IEOR(A, BC%Pack_I32(P, Offset) + B)
            ...
            X = IEOR(X, BC%Pack_I32(P, Length - 8))
        ...
        ELSE
            IF (IAND(ToInteger(Length)-1, 4) /= 0) THEN
                ...
                A = A*M5
            END IF
            X = IEOR(X, BC%Pack_I32(P, Length - 4) + Y)
...
END SUBMODULE SubBase_NMHashV2_Ref
