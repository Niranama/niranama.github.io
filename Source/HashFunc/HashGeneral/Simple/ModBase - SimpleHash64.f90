...
!** MODULE PARAMETERS:
    ...
    ! kind of hash code
    tInteger,  PARAMETER    :: kHash = kInt64
...
!** INTERFACE DEFINITIONS:
    ABSTRACT INTERFACE
        PURE FUNCTION GetKey(KeyByte) RESULT(KeyNum)
            ...
            IMPORT
            tByte, INTENT(IN)   :: KeyByte  ! key as a byte
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(*), DIMENSION(..), INTENT(IN)  :: Key          !! key (any type and rank)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    tHash, PARAMETER    :: HighBits         = SHIFTL(ToLong(Z'FFFFFFFFFFFFFFFF'), (HashBits-OneEighthBits))
...
!** FLOW
    ...
    ELSE
        GetKeyNum => Get_KeyNum_BE
    END IF
    SET_OPTION(HashCode, ToLong(Z'00000000AAAAAAAA'), StartHash)
    CALL AnyType_2_ByteArrPtr(Key, KeySize, KeyPtr)

    ! perform hashing
    DO I = 1, SIZE(KeyPtr, KIND=kIndex)
        IF (IAND(I-1, 1) == 0) THEN
...
!** FLOW
    ...
    DO I = 1, SIZE(KeyPtr, KIND=kIndex)
        ...
        HashCode = IEOR(SHIFTR(HashCode, 30), HashCode) * ToLong(Z'BF58476D1CE4E5B9')
        HashCode = IEOR(SHIFTR(HashCode, 27), HashCode) * ToLong(Z'94D049BB133111EB')
...
!** FLOW
    ...
    KeyNum = IAND(ToLong(KeyByte), ToLong(Z'00000000000000FF'))

    ! alternative implementation
    !KeyNum = TRANSFER([KeyByte, 0_kByte, 0_kByte, 0_kByte,  &
    !                   0_kByte, 0_kByte, 0_kByte, 0_kByte], 0_kHash)
...
!** FLOW
    ...
    KeyNum = SHIFTL(IAND(ToLong(KeyByte), ToLong(Z'00000000000000FF')), 56)

    ! alternative implementation
    !KeyNum = TRANSFER([0_kByte, 0_kByte, 0_kByte, KeyByte], 0_kHash)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    tByte, INTENT(IN)   :: KeyByte  ! key in (integer) byte
...
!** FLOW
    ...
    IF (IsLittleEndian) THEN
        KeyNum = TRANSFER([KeyByte, 0_kByte, 0_kByte, 0_kByte,  &
                            0_kByte, 0_kByte, 0_kByte, 0_kByte], 0_kHash)
    ELSE
        KeyNum = TRANSFER([0_kByte, 0_kByte, 0_kByte, 0_kByte,  &
                            0_kByte, 0_kByte, 0_kByte, KeyByte], 0_kHash)
    END IF
...
END MODULE ModBase_SimpleHash64
