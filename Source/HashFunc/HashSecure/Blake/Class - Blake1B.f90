...
!** MODULE PARAMETERS:
    ...
    tLong,  PARAMETER   :: CS(0:15) = [                               &
            ToLong(Z'243F6A8885A308D3'), ToLong(Z'13198A2E03707344'), &
            ToLong(Z'A4093822299F31D0'), ToLong(Z'082EFA98EC4E6C89'), &
            ToLong(Z'452821E638D01377'), ToLong(Z'BE5466CF34E90C6C'), &
            ToLong(Z'C0AC29B7C97C50DD'), ToLong(Z'3F84D5B5B5470917'), &
            ToLong(Z'9216D5D98979FB1B'), ToLong(Z'D1310BA698DFB5AC'), &
            ToLong(Z'2FFD72DBD01ADFB7'), ToLong(Z'B8E1AFED6A267E96'), &
            ToLong(Z'BA7C9045F12C7F99'), ToLong(Z'24A19947B3916CF7'), &
            ToLong(Z'0801F2E2858EFC16'), ToLong(Z'636920D871574E69')]
...
! FLOW
    ...
    ! initialize
    MD%T(0) = MD%T(0) + 1024_kLong
    IF (IAND(MD%T(0), NOT(ToLong(Z'00000000000003FF'))) == 0_kLong) MD%T(1) = MD%T(1) + 1
...
! FLOW
    ...
    ASSOCIATE(TmpBuf => MD%BufArr)   
        ...
        IF ((Ptr == 0_kIndex).AND.(NBits == 0_kIndex)) THEN
            MD%T(0) = ToLong(Z'FFFFFFFFFFFFFC00')
            MD%T(1) = ToLong(Z'FFFFFFFFFFFFFFFF')
        ELSEIF (MD%T(0) == 0) THEN
            MD%T(0) = ToLong(Z'FFFFFFFFFFFFFC00') + ToLong(BitLen)
            MD%T(1) = MD%T(1) - 1_kLong
        ELSE
            MD%T(0) = MD%T(0) - ToLong(1024 - BitLen)
        END IF
