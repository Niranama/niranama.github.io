...
!** MODULE PARAMETERS:
    ...
    !---------------------------------------------------------------------
    tByte,    PARAMETER :: CHUNK_START         = SHIFTL(1_kByte, 0)
    tByte,    PARAMETER :: CHUNK_END           = SHIFTL(1_kByte, 1)
    tByte,    PARAMETER :: PARENT              = SHIFTL(1_kByte, 2)
    tByte,    PARAMETER :: ROOT                = SHIFTL(1_kByte, 3)
    tByte,    PARAMETER :: KEYED_HASH          = SHIFTL(1_kByte, 4)
    tByte,    PARAMETER :: DERIVE_KEY_CONTEXT  = SHIFTL(1_kByte, 5)
    tByte,    PARAMETER :: DERIVE_KEY_MATERIAL = SHIFTL(1_kByte, 6)
    !---------------------------------------------------------------------
...
!** FLOW
    ...
    CALL ChunkState_Reset(MD%Chunk, MD%Key, 0_kLong)
...
! FLOW
    ...
    CALL MD%DoFinal(0_kLong, ByteArr, HashLen)
    CALL MD%Reset()
...
! FLOW
    ...
    IF (OutputLen > 0_kIndex) THEN
        CALL MD%DoFinal(0_kLong, ByteArr, OutputLen)
        CALL MD%Reset()
    END IF
