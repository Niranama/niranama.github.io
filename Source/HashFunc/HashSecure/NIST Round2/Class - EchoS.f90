...
! FLOW
    ...
    MD%C = 0
    MD%V = 0
    MD%V(0)  = ToInteger(SHIFTL(MD%GetDigestLen(), 3))
    MD%V(4)  = MD%V(0)
    MD%V(8)  = MD%V(0)
    MD%V(12) = MD%V(0)
    CALL MD%EngineReset()
...
! FLOW
    ...
    ASSOCIATE(TmpBuf => MD%BufArr)
        ...
        TmpBuf(174) = ToByte(OutLen)
        TmpBuf(175) = ToByte(SHIFTR(OutLen, 8))
...
CONTAINS
    ...
    !** SUBROUTINE PARAMETER DECLARATIONS:
        tInteger, PARAMETER :: C1 = ToInteger(Z'80808080')
        tInteger, PARAMETER :: C2 = ToInteger(Z'7F7F7F7F')
