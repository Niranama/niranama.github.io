...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: IV384_BE(0:15) = [           &
        ToInteger(Z'71F48510'), ToInteger(Z'A903A8AC'), & 
        ToInteger(Z'FE3216DD'), ToInteger(Z'0B2D2AD4'), &
        ToInteger(Z'6672900A'), ToInteger(Z'41032819'), & 
        ToInteger(Z'15A7D780'), ToInteger(Z'B3CAB8D9'), &
        ToInteger(Z'34EF4711'), ToInteger(Z'DE019FE8'), & 
        ToInteger(Z'4D674DC4'), ToInteger(Z'E056D96B'), &
        ToInteger(Z'A35C016B'), ToInteger(Z'DD903BA7'), & 
        ToInteger(Z'8C1B09B4'), ToInteger(Z'2C3E9F25')]
    tInteger, PARAMETER :: IV384_LE(0:15) = [           &
        ToInteger(Z'83DF1545'), ToInteger(Z'F9AAEC13'), & 
        ToInteger(Z'F4803CB0'), ToInteger(Z'11FE1F47'), &
        ToInteger(Z'DA6CD269'), ToInteger(Z'4F53FCD7'), & 
        ToInteger(Z'950529A2'), ToInteger(Z'97908147'), &
        ToInteger(Z'B0A4D7AF'), ToInteger(Z'2B9132BF'), & 
        ToInteger(Z'226E607D'), ToInteger(Z'3C0F8D7C'), &
        ToInteger(Z'487B3F0F'), ToInteger(Z'04363E22'), & 
        ToInteger(Z'0155C99C'), ToInteger(Z'EC2E20D3')]
    tInteger, PARAMETER :: IV512_BE(0:15) = [           &
        ToInteger(Z'D5652B63'), ToInteger(Z'25F1E6EA'), & 
        ToInteger(Z'B18F48FA'), ToInteger(Z'A1EE3A47'), &
        ToInteger(Z'C8B67B07'), ToInteger(Z'BDCE48D3'), & 
        ToInteger(Z'E3937B78'), ToInteger(Z'05DB5186'), &
        ToInteger(Z'613BE326'), ToInteger(Z'A11FA303'), & 
        ToInteger(Z'90C833D4'), ToInteger(Z'79CEE316'), &
        ToInteger(Z'1E1AF00F'), ToInteger(Z'2829B165'), & 
        ToInteger(Z'23B25F80'), ToInteger(Z'21E11499')]
    tInteger, PARAMETER :: IV512_LE(0:15) = [           &
        ToInteger(Z'72FCCDD8'), ToInteger(Z'79CA4727'), & 
        ToInteger(Z'128A077B'), ToInteger(Z'40D55AEC'), &
        ToInteger(Z'D1901A06'), ToInteger(Z'430AE307'), & 
        ToInteger(Z'B29F5CD1'), ToInteger(Z'DF07FBFC'), &
        ToInteger(Z'8E45D73D'), ToInteger(Z'681AB538'), & 
        ToInteger(Z'BDE86578'), ToInteger(Z'DD577E47'), &
        ToInteger(Z'E275EADE'), ToInteger(Z'502D9FCD'), & 
        ToInteger(Z'B9357178'), ToInteger(Z'022A4B9A')]
...
! FLOW
    ...
    BC = MD%GetBlockCount() + 1_kLong
    BitLen = SHIFTL(BC, 10)
    Cnt0 = ToInteger(BitLen)
    Cnt1 = ToInteger(SHIFTR(BitLen, 32))
    Cnt2 = ToInteger(SHIFTR(BC, 54))
...
! FLOW
    ...
    ASSOCIATE(TmpBuf => MD%BufArr)
        ...
        Cnt0 = ToInteger(BitLen) + NBits
        Cnt1 = ToInteger(SHIFTR(BitLen, 32))
        Cnt2 = ToInteger(SHIFTR(BC, 54))
        Z = SHIFTR(FByte80, NBits)
        Z = IAND(IOR(IAND(LastByte, -Z), Z), FByteFF)
        IF ((Ptr == 0_kIndex).AND.(NBits == 0_kByte)) THEN
            TmpBuf(0) = FByte80
        ...
        ELSE
            ...
            Cnt2 = 0
        END IF
        CALL ByteUnpackLE(ToInteger(BitLen) + NBits, TmpBuf, 110_kIndex)
        CALL ByteUnpackLE(ToInteger(SHIFTR(BitLen, 32)), TmpBuf, 114_kIndex)
        CALL ByteUnpackLE(ToInteger(SHIFTR(BC, 54)), TmpBuf, 118_kIndex)
        TmpBuf(122:125) = FByte00
        DLen = MD%GetDigestLen()
        TmpBuf(126) = ToByte(SHIFTL(DLen, 3))
        TmpBuf(127) = ToByte(SHIFTR(DLen, 5))
        CALL MD%Process(MD%H, TmpBuf, Cnt0, Cnt1, Cnt2)
    END ASSOCIATE
