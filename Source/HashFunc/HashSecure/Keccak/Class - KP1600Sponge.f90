...
!** USE STATEMENTS:
    ...
    USE ModBase_UIntUtil,           ONLY: OPERATOR (.UGE.)
...
!** MODULE PARAMETERS:
    ...
    tByte,    PARAMETER :: FByte01 = ToByte(Z'01')
    ...
    ! return flags
    tByte,    PARAMETER :: SUCCESS           = 0_kByte
    tByte,    PARAMETER :: FAILURE           = 1_kByte
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE KP1600Sponge
        ...
        tByte           :: State(0:KP1600SizeInBytes-1) = 0_kByte
...
! FLOW
    ...
    DO WHILE (I < InpByteLen)
        ...
        ELSE
            ...
            PartialBlock = ToInteger(InpByteLen - I)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    tLong,    PARAMETER :: KeccakRoundConstants(0:MaxNrRounds-1) = [  &
            ToLong(Z'0000000000000001'), ToLong(Z'0000000000008082'), &
            ToLong(Z'800000000000808A'), ToLong(Z'8000000080008000'), &
            ToLong(Z'000000000000808B'), ToLong(Z'0000000080000001'), &
            ToLong(Z'8000000080008081'), ToLong(Z'8000000000008009'), &
            ToLong(Z'000000000000008A'), ToLong(Z'0000000000000088'), &
            ToLong(Z'0000000080008009'), ToLong(Z'000000008000000A'), &
            ToLong(Z'000000008000808B'), ToLong(Z'800000000000008B'), &
            ToLong(Z'8000000000008089'), ToLong(Z'8000000000008003'), &
            ToLong(Z'8000000000008002'), ToLong(Z'8000000000000080'), &
            ToLong(Z'000000000000800A'), ToLong(Z'800000008000000A'), &
            ToLong(Z'8000000080008081'), ToLong(Z'8000000000008080'), &
            ToLong(Z'0000000080000001'), ToLong(Z'8000000080008008')]
...
! FLOW
    ...
    DO WHILE (I < OutByteLen)
        IF ((Instance%ByteIOIndex == RateInBytes).AND.(OutByteLen >= (I + RateInBytes))) THEN
            ...
            DO WHILE (J >= RateInBytes)
                CALL Instance%Permute()
                CALL Instance%ExtractBytes(OutDat(CurIndx:), 0_kIndex, RateInBytes)
                CurIndx = CurIndx + RateInBytes
                J = J - RateInBytes
            END DO
            I = OutByteLen - J
        ELSE
            ...
            IF (Instance%ByteIOIndex == RateInBytes) THEN
                ...
                Instance%ByteIOIndex = 0
            END IF
            PartialBlock = ToInteger(OutByteLen - I)
            IF (PartialBlock+Instance%ByteIOIndex > RateInBytes) THEN
                PartialBlock = RateInBytes - Instance%ByteIOIndex
            END IF
            I = I + PartialBlock
...
! FLOW
    ...
    ELSE
        BLOCK
            ...
            DO WHILE (I < Length)
                Lane = InpBytes(I)
                Lane = SHIFTL(Lane, MOD(Offset+I, 8)*8)
                Indx = SHIFTR(Offset+I, 3)
                Instance%StateAsWords(Indx) = IEOR(Instance%StateAsWords(Indx), Lane)
            END DO
        END BLOCK
