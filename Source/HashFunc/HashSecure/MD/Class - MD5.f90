...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: IV(0:3) = [                      &
            ToInteger(Z'67452301'), ToInteger(Z'EFCDAB89'), &
            ToInteger(Z'98BADCFE'), ToInteger(Z'10325476')]
...
! FLOW
    ...
    A = B + RotateLeft(A + FRound(B, C, D) + X( 0) + ToInteger(Z'D76AA478'), 7)
    D = A + RotateLeft(D + FRound(A, B, C) + X( 1) + ToInteger(Z'E8C7B756'), 12)
    C = D + RotateLeft(C + FRound(D, A, B) + X( 2) + ToInteger(Z'242070DB'), 17)
    B = C + RotateLeft(B + FRound(C, D, A) + X( 3) + ToInteger(Z'C1BDCEEE'), 22)
    A = B + RotateLeft(A + FRound(B, C, D) + X( 4) + ToInteger(Z'F57C0FAF'), 7)
    D = A + RotateLeft(D + FRound(A, B, C) + X( 5) + ToInteger(Z'4787C62A'), 12)
    C = D + RotateLeft(C + FRound(D, A, B) + X( 6) + ToInteger(Z'A8304613'), 17)
    B = C + RotateLeft(B + FRound(C, D, A) + X( 7) + ToInteger(Z'FD469501'), 22)
    A = B + RotateLeft(A + FRound(B, C, D) + X( 8) + ToInteger(Z'698098D8'), 7)
    D = A + RotateLeft(D + FRound(A, B, C) + X( 9) + ToInteger(Z'8B44F7AF'), 12)
    C = D + RotateLeft(C + FRound(D, A, B) + X(10) + ToInteger(Z'FFFF5BB1'), 17)
    B = C + RotateLeft(B + FRound(C, D, A) + X(11) + ToInteger(Z'895CD7BE'), 22)
    A = B + RotateLeft(A + FRound(B, C, D) + X(12) + ToInteger(Z'6B901122'), 7)
    D = A + RotateLeft(D + FRound(A, B, C) + X(13) + ToInteger(Z'FD987193'), 12)
    C = D + RotateLeft(C + FRound(D, A, B) + X(14) + ToInteger(Z'A679438E'), 17)
    B = C + RotateLeft(B + FRound(C, D, A) + X(15) + ToInteger(Z'49B40821'), 22)

    A = B + RotateLeft(A + GRound(B, C, D) + X( 1) + ToInteger(Z'F61E2562'), 5)
    D = A + RotateLeft(D + GRound(A, B, C) + X( 6) + ToInteger(Z'C040B340'), 9)
    C = D + RotateLeft(C + GRound(D, A, B) + X(11) + ToInteger(Z'265E5A51'), 14)
    B = C + RotateLeft(B + GRound(C, D, A) + X( 0) + ToInteger(Z'E9B6C7AA'), 20)
    A = B + RotateLeft(A + GRound(B, C, D) + X( 5) + ToInteger(Z'D62F105D'), 5)
    D = A + RotateLeft(D + GRound(A, B, C) + X(10) + ToInteger(Z'02441453'), 9)
    C = D + RotateLeft(C + GRound(D, A, B) + X(15) + ToInteger(Z'D8A1E681'), 14)
    B = C + RotateLeft(B + GRound(C, D, A) + X( 4) + ToInteger(Z'E7D3FBC8'), 20)
    A = B + RotateLeft(A + GRound(B, C, D) + X( 9) + ToInteger(Z'21E1CDE6'), 5)
    D = A + RotateLeft(D + GRound(A, B, C) + X(14) + ToInteger(Z'C33707D6'), 9)
    C = D + RotateLeft(C + GRound(D, A, B) + X( 3) + ToInteger(Z'F4D50D87'), 14)
    B = C + RotateLeft(B + GRound(C, D, A) + X( 8) + ToInteger(Z'455A14ED'), 20)
    A = B + RotateLeft(A + GRound(B, C, D) + X(13) + ToInteger(Z'A9E3E905'), 5)
    D = A + RotateLeft(D + GRound(A, B, C) + X( 2) + ToInteger(Z'FCEFA3F8'), 9)
    C = D + RotateLeft(C + GRound(D, A, B) + X( 7) + ToInteger(Z'676F02D9'), 14)
    B = C + RotateLeft(B + GRound(C, D, A) + X(12) + ToInteger(Z'8D2A4C8A'), 20)

    A = B + RotateLeft(A + HRound(B, C, D) + X( 5) + ToInteger(Z'FFFA3942'), 4)
    D = A + RotateLeft(D + HRound(A, B, C) + X( 8) + ToInteger(Z'8771F681'), 11)
    C = D + RotateLeft(C + HRound(D, A, B) + X(11) + ToInteger(Z'6D9D6122'), 16)
    B = C + RotateLeft(B + HRound(C, D, A) + X(14) + ToInteger(Z'FDE5380C'), 23)
    A = B + RotateLeft(A + HRound(B, C, D) + X( 1) + ToInteger(Z'A4BEEA44'), 4)
    D = A + RotateLeft(D + HRound(A, B, C) + X( 4) + ToInteger(Z'4BDECFA9'), 11)
    C = D + RotateLeft(C + HRound(D, A, B) + X( 7) + ToInteger(Z'F6BB4B60'), 16)
    B = C + RotateLeft(B + HRound(C, D, A) + X(10) + ToInteger(Z'BEBFBC70'), 23)
    A = B + RotateLeft(A + HRound(B, C, D) + X(13) + ToInteger(Z'289B7EC6'), 4)
    D = A + RotateLeft(D + HRound(A, B, C) + X( 0) + ToInteger(Z'EAA127FA'), 11)
    C = D + RotateLeft(C + HRound(D, A, B) + X( 3) + ToInteger(Z'D4EF3085'), 16)
    B = C + RotateLeft(B + HRound(C, D, A) + X( 6) + ToInteger(Z'04881D05'), 23)
    A = B + RotateLeft(A + HRound(B, C, D) + X( 9) + ToInteger(Z'D9D4D039'), 4)
    D = A + RotateLeft(D + HRound(A, B, C) + X(12) + ToInteger(Z'E6DB99E5'), 11)
    C = D + RotateLeft(C + HRound(D, A, B) + X(15) + ToInteger(Z'1FA27CF8'), 16)
    B = C + RotateLeft(B + HRound(C, D, A) + X( 2) + ToInteger(Z'C4AC5665'), 23)

    A = B + RotateLeft(A + IRound(B, C, D) + X( 0) + ToInteger(Z'F4292244'), 6)
    D = A + RotateLeft(D + IRound(A, B, C) + X( 7) + ToInteger(Z'432AFF97'), 10)
    C = D + RotateLeft(C + IRound(D, A, B) + X(14) + ToInteger(Z'AB9423A7'), 15)
    B = C + RotateLeft(B + IRound(C, D, A) + X( 5) + ToInteger(Z'FC93A039'), 21)
    A = B + RotateLeft(A + IRound(B, C, D) + X(12) + ToInteger(Z'655B59C3'), 6)
    D = A + RotateLeft(D + IRound(A, B, C) + X( 3) + ToInteger(Z'8F0CCC92'), 10)
    C = D + RotateLeft(C + IRound(D, A, B) + X(10) + ToInteger(Z'FFEFF47D'), 15)
    B = C + RotateLeft(B + IRound(C, D, A) + X( 1) + ToInteger(Z'85845DD1'), 21)
    A = B + RotateLeft(A + IRound(B, C, D) + X( 8) + ToInteger(Z'6FA87E4F'), 6)
    D = A + RotateLeft(D + IRound(A, B, C) + X(15) + ToInteger(Z'FE2CE6E0'), 10)
    C = D + RotateLeft(C + IRound(D, A, B) + X( 6) + ToInteger(Z'A3014314'), 15)
    B = C + RotateLeft(B + IRound(C, D, A) + X(13) + ToInteger(Z'4E0811A1'), 21)
    A = B + RotateLeft(A + IRound(B, C, D) + X( 4) + ToInteger(Z'F7537E82'), 6)
    D = A + RotateLeft(D + IRound(A, B, C) + X(11) + ToInteger(Z'BD3AF235'), 10)
    C = D + RotateLeft(C + IRound(D, A, B) + X( 2) + ToInteger(Z'2AD7D2BB'), 15)
    B = C + RotateLeft(B + IRound(C, D, A) + X( 9) + ToInteger(Z'EB86D391'), 21)
