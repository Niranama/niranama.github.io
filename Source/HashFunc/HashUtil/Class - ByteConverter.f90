...
!   (an array of 8-bit integers) array and an other integer type (i.e. a 16-bit,
...
!** INTERFACE DEFINITIONS:
    ABSTRACT INTERFACE
        ...
        FUNCTION PackShort(Buf, Off) RESULT(Res)
            ...
            IMPORT
            tByte,  INTENT(IN)  :: Buf(0:)  !! byte sequence
        ...
        SUBROUTINE UnpackShort(Val, Buf, Off)
            ...
            IMPORT
            tShort, INTENT(IN)      :: Val      !! the value to convert
        ...
        SUBROUTINE UnpackInt(Val, Buf, Off)
            ...
            IMPORT
            tInteger, INTENT(IN)    :: Val      !! the value to convert
            tByte,    INTENT(INOUT) :: Buf(0:)  !! the destination buffer
            tIndex,   INTENT(IN)    :: Off      !! the destination offset
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger                            :: OutVal   !! the requested byte, widened to default integer type
...
!** FLOW
    ...
    OutVal = IAND(BC%Get_I8(Buf, Offset), ToInteger(Z'000000FF'))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tByte,                INTENT(IN)    :: Buf(0:)  !! byte sequence
    tIndex,               INTENT(IN)    :: Offset   !! offset to the byte to read within the byte sequence
    tInteger                            :: OutVal   !! the requested result, widened to default integer type
...
!** FLOW
    ...
    OutVal = IAND(ToInteger(BC%Pack_I16(Buf, Offset)), ToInteger(Z'0000FFFF'))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tByte,                INTENT(IN)    :: Buf(0:)  !! byte sequence
    tIndex,               INTENT(IN)    :: Offset   !! offset to the byte to read within the byte sequence
    tLong                               :: OutVal   !! the requested result, widened to long integer type
...
!** FLOW
    ...
    OutVal = IAND(ToLong(BC%Pack_I32(Buf, Offset)), ToLong(Z'00000000FFFFFFFF'))
...
! FLOW
    ...
    Res = IOR(SHIFTL(MaskShort(Buf(Off)), 8), MaskShort(Buf(Off+1)))

    ! implementation algorithm #2 (comparable to #1)
!    Res = SHIFTL(MaskShort(Buf(Off)), 8) + MaskShort(Buf(Off+1))
...
!    Res = 0
!    CALL MVBITS(MaskShort(Buf(Off)), 0, 8, Res, 8)
!    CALL MVBITS(MaskShort(Buf(Off+1)), 0, 8, Res, 0)

    ! implementation algorithm #5 (slowest)
!    Res = TRANSFER([Buf(Off+1), Buf(Off)], 0_kShort)
...
! FLOW
    ...
    ! implementation algorithm #1
    Res = IOR(IOR(IOR(SHIFTL(MaskInteger(Buf(Off)),   24),  &
                      SHIFTL(MaskInteger(Buf(Off+1)), 16)), &
                      SHIFTL(MaskInteger(Buf(Off+2)),  8)), &
                             MaskInteger(Buf(Off+3)))

    ! implementation algorithm #2 (comparable to #1)
!    Res = SHIFTL(MaskInteger(Buf(Off)),   24) + &
!          SHIFTL(MaskInteger(Buf(Off+1)), 16) + &
!          SHIFTL(MaskInteger(Buf(Off+2)),  8) + &
!                 MaskInteger(Buf(Off+3))
...
!    Res = 0
!    DO I = 0, 3
!        CALL MVBITS(MaskInteger(Buf(Off+I)), 0, 8, Res, 24-I*8)
!    END DO

    ! implementation algorithm #5 (slowest)
!    Res = TRANSFER([Buf(Off+3), Buf(Off+2), Buf(Off+1), Buf(Off)], 0_kInteger)
...
!    Res = IOR(IOR(IOR(IOR(IOR(IOR(IOR(SHIFTL(MaskLong(Buf(Off)),   56),   &
!                                      SHIFTL(MaskLong(Buf(Off+1)), 48)),  &
!                                      SHIFTL(MaskLong(Buf(Off+2)), 40)),  &
!                                      SHIFTL(MaskLong(Buf(Off+3)), 32)),  &
!                                      SHIFTL(MaskLong(Buf(Off+4)), 24)),  &
!                                      SHIFTL(MaskLong(Buf(Off+5)), 16)),  &
!                                      SHIFTL(MaskLong(Buf(Off+6)),  8)),  &
!                                             MaskLong(Buf(Off+7)))

    ! implementation algorithm #2 (comparable to #1)
!    Res = SHIFTL(MaskLong(Buf(Off)),   56) + &
!          SHIFTL(MaskLong(Buf(Off+1)), 48) + &
!          SHIFTL(MaskLong(Buf(Off+2)), 40) + &
!          SHIFTL(MaskLong(Buf(Off+3)), 32) + &
!          SHIFTL(MaskLong(Buf(Off+4)), 24) + &
...
!    Res = 0
!    DO I = 0, 7
!        CALL MVBITS(MaskLong(Buf(Off+I)), 0, 8, Res, 56-I*8)
!    END DO

    ! implementation algorithm #5 (slowest)
!    Res = TRANSFER([Buf(Off+7), Buf(Off+6), Buf(Off+5), Buf(Off+4), &
!                   Buf(Off+3), Buf(Off+2), Buf(Off+1), Buf(Off)], 0_kLong)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    tShort, INTENT(IN)      :: Val      !! the value to encode
    tByte,  INTENT(INOUT)   :: Buf(0:)  !! the destination buffer
    tIndex, INTENT(IN)      :: Off      !! the destination offset
...
! FLOW
    ...
    ! implementation algorithm #1
    Buf(Off)   = ToByte(SHIFTR(Val, 8))
    Buf(Off+1) = ToByte(Val)

    ! implementation algorithm #2 (comparable to #1)
!    Buf(Off)   = ToByte(IBITS(Val, 8, 8))
!    Buf(Off+1) = ToByte(IBITS(Val, 0, 8))
...
! FLOW
    ...
    ! implementation algorithm #1
    Buf(Off)   = ToByte(SHIFTR(Val, 24))
    Buf(Off+1) = ToByte(SHIFTR(Val, 16))
    Buf(Off+2) = ToByte(SHIFTR(Val, 8))
    Buf(Off+3) = ToByte(Val)

    ! implementation algorithm #2 (comparable to #1)
!    Buf(Off)   = ToByte(IBITS(Val, 24, 8))
!    Buf(Off+1) = ToByte(IBITS(Val, 16, 8))
!    Buf(Off+2) = ToByte(IBITS(Val,  8, 8))
!    Buf(Off+3) = ToByte(IBITS(Val,  0, 8))
...
! FLOW
    ...
    ! implementation algorithm #1
    Buf(Off)   = ToByte(SHIFTR(Val, 56))
    Buf(Off+1) = ToByte(SHIFTR(Val, 48))
    Buf(Off+2) = ToByte(SHIFTR(Val, 40))
    Buf(Off+3) = ToByte(SHIFTR(Val, 32))
    Buf(Off+4) = ToByte(SHIFTR(Val, 24))
    Buf(Off+5) = ToByte(SHIFTR(Val, 16))
    Buf(Off+6) = ToByte(SHIFTR(Val, 8))
    Buf(Off+7) = ToByte(Val)

    ! implementation algorithm #2 (comparable to #1)
!    Buf(Off)   = ToByte(IBITS(Val, 56, 8))
!    Buf(Off+1) = ToByte(IBITS(Val, 48, 8))
!    Buf(Off+2) = ToByte(IBITS(Val, 40, 8))
!    Buf(Off+3) = ToByte(IBITS(Val, 32, 8))
!    Buf(Off+4) = ToByte(IBITS(Val, 24, 8))
!    Buf(Off+5) = ToByte(IBITS(Val, 16, 8))
!    Buf(Off+6) = ToByte(IBITS(Val,  8, 8))
!    Buf(Off+7) = ToByte(IBITS(Val,  0, 8))
...
! FLOW
    ...
    ! implementation algorithm #1
    Res = IOR(MaskShort(Buf(Off)), SHIFTL(MaskShort(Buf(Off+1)), 8))

    ! implementation algorithm #2 (comparable to #1)
!    Res = MaskShort(Buf(Off)) + SHIFTL(MaskShort(Buf(Off+1)), 8)
...
!#undef UnsignedShort
    ...
    ! initialize because that argument of MVBITS is INTENT(INOUT)
!    Res = 0
!    CALL MVBITS(MaskShort(Buf(Off)), 0, 8, Res, 0)
!    CALL MVBITS(MaskShort(Buf(Off+1)), 0, 8, Res, 8)

    ! implementation algorithm #5 (slowest)
!    Res = TRANSFER([Buf(Off), Buf(Off+1)], 0_kShort)
...
!    Res = 0
!    DO I = 0, 3
!        CALL MVBITS(MaskInteger(Buf(Off+I)), 0, 8, Res, I*8)
!    END DO

    ! implementation algorithm #5 (slowest)
!    Res = TRANSFER([Buf(Off), Buf(Off+1), Buf(Off+2), Buf(Off+3)], 0_kInteger)
...
!    Res = 0
!    DO I = 0, 7
!        CALL MVBITS(MaskLong(Buf(Off+I)), 0, 8, Res, I*8)
!    END DO

    ! implementation algorithm #5 (slowest)
!    Res = TRANSFER([Buf(Off), Buf(Off+1), Buf(Off+2), Buf(Off+3), &
!                   Buf(Off+4), Buf(Off+5), Buf(Off+6), Buf(Off+7)], 0_kLong)
...
! FLOW
    ...
    ! implementation algorithm #2 (comparable to #1)
!    Buf(Off)   = ToByte(IBITS(Val,  0, 8))
!    Buf(Off+1) = ToByte(IBITS(Val,  8, 8))
...
!    Buf(Off)   = ToByte(IBITS(Val,  0, 8))
!    Buf(Off+1) = ToByte(IBITS(Val,  8, 8))
!    Buf(Off+2) = ToByte(IBITS(Val, 16, 8))
!    Buf(Off+3) = ToByte(IBITS(Val, 24, 8))
...
!    Buf(Off+2) = ToByte(IBITS(Val, 16, 8))
!    Buf(Off+3) = ToByte(IBITS(Val, 24, 8))
!    Buf(Off+4) = ToByte(IBITS(Val, 32, 8))
!    Buf(Off+5) = ToByte(IBITS(Val, 40, 8))
!    Buf(Off+6) = ToByte(IBITS(Val, 48, 8))
!    Buf(Off+7) = ToByte(IBITS(Val, 56, 8))
...
END MODULE Class_ByteConverter
