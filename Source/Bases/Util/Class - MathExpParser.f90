...
!       The software package FEATFLOW2</a> <br>
    ...
    USE ModBase_DoublyLinkedLists,  ONLY: StackI4B => ListInteger4B
...
!** MODULE PARAMETERS:
    ...
    !--------------------------------------------------------------------------
    ! name of module
    tCharStar, PARAMETER        :: ModName = 'Class_MathExpParser'
    ! Size of character variable for storing error messages returned from
    ! ALLOCATE and DEALLOCATE statement.
    tInteger, PARAMETER         :: MsgLen = 128
    ! constant for infinity
    tReal,    PARAMETER         :: SYS_INFINITY_kFP  = HUGE(1.0_kFP)
    !% Length of string
    tInteger, PARAMETER, PUBLIC :: FPAR_STR_LEN    = 2048
    !% Maximum number of predefined/user-defined constants
    tInteger, PARAMETER, PUBLIC :: FPAR_MAX_CONSTS = 128
    !% Maximum number of predefined/user-defined expressions
    tInteger, PARAMETER, PUBLIC :: FPAR_MAX_EXPRS  = 128
    !% Length of constant name
    tInteger, PARAMETER, PUBLIC :: FPAR_CONST_LEN  = 32
    !% Length of expression name
    tInteger, PARAMETER, PUBLIC :: FPAR_EXPR_LEN   = 32
    !% Length of function name
    tInteger, PARAMETER, PUBLIC :: FPAR_FUNC_LEN   = 32
    !% Length of variable name
    tInteger, PARAMETER, PUBLIC :: FPAR_VAR_LEN    = 32
    !--------------------------------------------------------------------------
    !-----                  types for parser expressions                  -----
    !--------------------------------------------------------------------------
    !% Constant
    tInteger, PARAMETER, PUBLIC :: FPAR_CONSTANT   = 1
    !% Expression
    tInteger, PARAMETER, PUBLIC :: FPAR_EXPRESSION = 2
    !% Functions (including symbolic variables)
    tInteger, PARAMETER, PUBLIC :: FPAR_FUNCTION   = 3
    !--------------------------------------------------------------------------
    !-----                      keywords for parser                       -----
    !--------------------------------------------------------------------------
    tByte,    PARAMETER ::    cImmed       =  1, &
                              cJump        =  2, &
                              cNeg         =  3, &
                              cDeg         =  4, &
                              cRad         =  5, &
    ...
    tByte,    PARAMETER ::    cIf          = 21, & ! <-- if-then-else
    ...
    !--------------------------------------------------------------------------
    tReal,                    PARAMETER :: PredefinedConstVals(3) = [      &
                3.141592653589793115997963468544185161590576171875_kFP,    &
                2.718281828459045090795598298427648842334747314453125_kFP, &
                SYS_INFINITY_kFP]
    !--------------------------------------------------------------------------
    ...
    !-----                        types of error                          -----
    !--------------------------------------------------------------------------
    tInteger, PARAMETER :: No_Error              = 0
    tInteger, PARAMETER :: DivisionByZero        = 1
    tInteger, PARAMETER :: IllegalArgument       = 2
    tInteger, PARAMETER :: ComplexValuedArgument = 3
    tInteger, PARAMETER :: IllegalOperation      = 4
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE ParserComponent
        PRIVATE
        !% Size of bytecode
        tInteger        :: iByteCodeSize = 0
        !% Size of immediates
        tInteger        :: iImmedSize = 0
        !% Stack size
        tInteger        :: iStackSize = 0
        !% Stack pointer
        tInteger        :: iStackPtr = 0
        ...
        !% Bytecode
        tByte, POINTER  :: iByteCode(:) => NULL()
        !% Immediates (real-valued)
        tReal, POINTER  :: dImmed(:)    => NULL()
        !% Immediates (complex-valued)
        tCmpx, POINTER  :: zImmed(:)    => NULL()
    ...
    TYPE MathExpParser
        ...
        tReal                           :: dConstantValue(FPAR_MAX_CONSTS) = Zero
        ! Global constant values for parser (complex-valued)
        tCmpx                           :: zConstantValue(FPAR_MAX_CONSTS) = (Zero, Zero)
...
SUBROUTINE Module_Documentation()
    ...
    !!#             in fsystem by SYS_INFINITY_kFP.
    !!# </verb>
    !!#
...
!** FLOW
    ...
    rfParser%dConstantValue = 0.0_kFP
    rfParser%zConstantValue = CMPLX(0.0_kFP, 0.0_kFP, KIND=kFP)
...
!** FLOW
    ...
    ASSOCIATE(nConstants     => rfParser%nConstants, &
        ...
        zConstantValue(nConstants) = CMPLX(dValue, 0.0_kFP, KIND=kFP)
    END ASSOCIATE
...
!** FLOW
    ...
    CALL MemAlloc(ToIndex(FPAR_FUNC_LEN), rfParser%SCompName, ToIndex(MaxNComp))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    !% Function identifier
    tInteger, OPTIONAL,   INTENT(OUT)   :: iComp
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tReal,                INTENT(IN)    :: dValue(:)
    !% Error flag
    tInteger,             INTENT(OUT)   :: EvalErrType
    !% Evaluated function
    tReal,                INTENT(OUT)   :: dResult
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% Error identifier
    tInteger, INTENT(IN)    :: EvalErrType
    !% Error messages
    tCharAlloc              :: sMessage
...
!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    ...
    tCharLen(5)                     :: n
    tInteger                        :: iinstPtr, idataPtr, iStackPtr, nparams
    tByte                           :: iOpcode
...
!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    ...
    tInteger        :: StackItem, idummy
    tByte           :: n, iopSize
    tCharLen(1)     :: c
    tReal           :: dnumber
    tLogical        :: bError, Success
    tInteger        :: iFunctionIndex, iFunctionIndex2
    tInteger        :: iparenthCount, ib, in, ifunctionLength
            ...
            PossibleError: BLOCK
                ...
                DO WHILE (c == ')')
                    IF (.NOT.rStack%IsEmpty()) THEN
                        Success = rStack%PeekTop(StackItem)
                        IF (StackItem == iparenthCount) THEN
                            Success = rStack%Pop(idummy)
                        END IF
                    END IF
                    iparenthCount = iparenthCount - 1
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% Index of function
    tByte, INTENT(IN)   :: iFunctionIndex
    !% Number if required parameters
    tInteger            :: nParameters
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger, OPTIONAL, INTENT(OUT) :: iBegin
    ! OPTIONAL: Position of character after name
    tInteger, OPTIONAL, INTENT(OUT) :: iNext
    ! Index of variable
    tByte                           :: n

!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    tInteger    :: j, ib, in, iStringLen
...
!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    tReal                   :: daux
    tInteger, ALLOCATABLE   :: p_Irandom1(:), p_Irandom2(:)
    tInteger                :: iaux
...
!** FLOW
    ...
    ! real-valued path is overwritten by SYS_INFINITY_kFP.
    SELECT CASE (iByte)
      !------------------------------------------------------------
      ! Functions
      !------------------------------------------------------------
    ...
    CASE (cAcos)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%dImmed(rComp%iImmedSize) < -1.0_kFP .OR. &
                rComp%dImmed(rComp%iImmedSize) >  1.0_kFP) rComp%bIsComplex = TrueVal

            ! Different treatment for real- and complex-valued case
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
    ...
    CASE (cAtan2)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed .AND.&
            ...
            rComp%zImmed(rComp%iImmedSize-1) = CMPLX(rComp%dImmed(rComp%iImmedSize-1),0.0_kFP,KIND=kFP)
    ...
    CASE (cAcot)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = ATAN(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            ELSE
                rComp%dImmed(rComp%iImmedSize) = ATAN(1.0_kFP/rComp%dImmed(rComp%iImmedSize))
                rComp%zImmed(rComp%iImmedSize) = ATAN(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            END IF
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cAcoth)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = ATANH(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            ELSE
                rComp%dImmed(rComp%iImmedSize) = ATANH(1.0_kFP/rComp%dImmed(rComp%iImmedSize))
                rComp%zImmed(rComp%iImmedSize) = ATANH(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            END IF
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cAcosh)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%dImmed(rComp%iImmedSize) < 1.0_kFP) rComp%bIsComplex = TrueVal
    ...
    CASE (cAcsc)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%dImmed(rComp%iImmedSize) > -Pi/2.0_kFP .AND. &
                rComp%dImmed(rComp%iImmedSize) <  Pi/2.0_kFP) rComp%bIsComplex = TrueVal

            ! Different treatment for real- and complex-valued case
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = ASIN(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            ELSE
                ! Real-valued path
                rComp%dImmed(rComp%iImmedSize) =  ASIN(1.0_kFP/rComp%dImmed(rComp%iImmedSize))
                rComp%zImmed(rComp%iImmedSize) = ASIN(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            END IF
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cAcsch)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = ASINH(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            ELSE
                ! Real-valued path
                rComp%dImmed(rComp%iImmedSize) = ASINH(1.0_kFP/rComp%dImmed(rComp%iImmedSize))
                rComp%zImmed(rComp%iImmedSize) = ASINH(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            END IF
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cAsec)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%dImmed(rComp%iImmedSize) > -1.0_kFP .AND. &
                rComp%dImmed(rComp%iImmedSize) <  1.0_kFP) rComp%bIsComplex = TrueVal

            ! Different treatment for real- and complex-valued case
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = ACOS(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            ELSE
                ! Real-valued path
                rComp%dImmed(rComp%iImmedSize) =  ACOS(1.0_kFP/rComp%dImmed(rComp%iImmedSize))
                rComp%zImmed(rComp%iImmedSize) = ACOS(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            END IF
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cAsech)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%dImmed(rComp%iImmedSize) < 0.0_kFP .OR. &
                rComp%dImmed(rComp%iImmedSize) > 1.0_kFP) rComp%bIsComplex = TrueVal

            ! Different treatment for real- and complex-valued case
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = ACOSH(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            ELSE
                ! Real-valued path
                rComp%dImmed(rComp%iImmedSize) = ACOSH(1.0_kFP/rComp%dImmed(rComp%iImmedSize))
                rComp%zImmed(rComp%iImmedSize) = ACOSH(1.0_kFP/rComp%zImmed(rComp%iImmedSize))
            END IF
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cAsinh)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%dImmed(rComp%iImmedSize) <= 0.0_kFP) rComp%bIsComplex = TrueVal
    ...
    CASE (cAnint)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            rComp%zImmed(rComp%iImmedSize) = CMPLX(rComp%dImmed(rComp%iImmedSize),0.0_kFP,KIND=kFP)
            CALL RemoveCompiledByte(rComp)
        END IF
        ...
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%bIsComplex) THEN
                ...
                rComp%zImmed(rComp%iImmedSize) = CMPLX(CEILING(REAL(rComp%zImmed(rComp%iImmedSize))),&
                                                       CEILING(AIMAG(rComp%zImmed(rComp%iImmedSize))),KIND=kFP)
            ELSE
        ...
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed .AND.&
            ...
            IF (rComp%bIsComplex) THEN
                ...
                rComp%zImmed(rComp%iImmedSize-1) = CMPLX(REAL(rComp%zImmed(rComp%iImmedSize-1)),&
                                                         REAL(rComp%zImmed(rComp%iImmedSize)),KIND=kFP)
                rComp%dImmed(rComp%iImmedSize-1) = SYS_INFINITY_kFP

            ELSE
                rComp%bIsComplex =TrueVal
                rComp%zImmed(rComp%iImmedSize-1) = CMPLX(rComp%dImmed(rComp%iImmedSize-1),&
                                                         rComp%dImmed(rComp%iImmedSize),KIND=kFP)
                rComp%dImmed(rComp%iImmedSize-1) = SYS_INFINITY_kFP
            END IF
            CALL RemoveCompiledImmediate(rComp)
            CALL RemoveCompiledByte(rComp)
    ...
    CASE (cCot)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            rComp%dImmed(rComp%iImmedSize) = 1.0_kFP/TAN(rComp%dImmed(rComp%iImmedSize))
            rComp%zImmed(rComp%iImmedSize) = 1.0_kFP/TAN(rComp%zImmed(rComp%iImmedSize))
            CALL RemoveCompiledByte(rComp)
        END IF

    CASE (cCoth)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            rComp%dImmed(rComp%iImmedSize) = 1.0_kFP/TANH(rComp%dImmed(rComp%iImmedSize))
            rComp%zImmed(rComp%iImmedSize) = 1.0_kFP/TANH(rComp%zImmed(rComp%iImmedSize))
            CALL RemoveCompiledByte(rComp)
        END IF

    CASE (cCsc)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            rComp%dImmed(rComp%iImmedSize) = 1.0_kFP/SIN(rComp%dImmed(rComp%iImmedSize))
            rComp%zImmed(rComp%iImmedSize) = 1.0_kFP/SIN(rComp%zImmed(rComp%iImmedSize))
            CALL RemoveCompiledByte(rComp)
        END IF

    CASE (cCsch)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            rComp%dImmed(rComp%iImmedSize) = 1.0_kFP/SINH(rComp%dImmed(rComp%iImmedSize))
            rComp%zImmed(rComp%iImmedSize) = 1.0_kFP/SINH(rComp%zImmed(rComp%iImmedSize))
            CALL RemoveCompiledByte(rComp)
        END IF
        ...
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%bIsComplex) THEN
                ...
                rComp%zImmed(rComp%iImmedSize) = CMPLX(FLOOR(REAL(rComp%zImmed(rComp%iImmedSize))),&
                                                       FLOOR(AIMAG(rComp%zImmed(rComp%iImmedSize))),KIND=kFP)
            ELSE
    ...
    CASE (cImag)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            rComp%zImmed(rComp%iImmedSize) = CMPLX(AIMAG(rComp%zImmed(rComp%iImmedSize)),0.0_kFP,KIND=kFP)
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cLog10)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            IF (rComp%bIsComplex) THEN
                rComp%dImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
                rComp%zImmed(rComp%iImmedSize) = LOG(rComp%zImmed(rComp%iImmedSize))/LOG(10.0_kFP)
            ELSE
    ...
    CASE (cReal)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            ...
            rComp%zImmed(rComp%iImmedSize) = CMPLX(REAL(rComp%zImmed(rComp%iImmedSize)),0.0_kFP,KIND=kFP)
            CALL RemoveCompiledByte(rComp)
        END IF
    ...
    CASE (cRrand)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed .AND.&
            ...
            rComp%zImmed(rComp%iImmedSize-1) = CMPLX(daux,0.0_kFP,KIND=kFP)
    ...
    CASE (cSec)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            rComp%dImmed(rComp%iImmedSize) = 1.0_kFP/COS(rComp%dImmed(rComp%iImmedSize))
            rComp%zImmed(rComp%iImmedSize) = 1.0_kFP/COS(rComp%zImmed(rComp%iImmedSize))
            CALL RemoveCompiledByte(rComp)
        END IF

    CASE (cSech)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
            rComp%dImmed(rComp%iImmedSize) = 1.0_kFP/COSH(rComp%dImmed(rComp%iImmedSize))
            rComp%zImmed(rComp%iImmedSize) = 1.0_kFP/COSH(rComp%zImmed(rComp%iImmedSize))
            CALL RemoveCompiledByte(rComp)
        END IF

    CASE (cSign)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed) THEN
          rComp%dImmed(rComp%iImmedSize) = SIGN(1.0_kFP,rComp%dImmed(rComp%iImmedSize))
    ...
    CASE (cDiv)
        IF (rComp%iByteCode(rComp%iByteCodeSize-1) == cImmed .AND.&
            ...
            ! Division by zero?
            IF (rComp%dImmed(rComp%iImmedSize) == 0.0_kFP) THEN
            ...
            ! Division by zero?
            IF (rComp%zImmed(rComp%iImmedSize) == CMPLX(0.0_kFP,0.0_kFP,KIND=kFP)) THEN
                ErrMsg = 'Division by zero!'
                CALL Handle_ErrLevel(SubName, ModName, ErrSevere, ErrMsg)
                RETURN
            END IF
...
!** FLOW
    ...
    rComp%zImmed(rComp%iImmedSize) = CMPLX(dImmediate, 0.0_kFP, KIND=kFP)
...
!** FLOW
    ...
    rComp%iImmedSize               = rComp%iImmedSize + 1
    rComp%zImmed(rComp%iImmedSize) = SYS_INFINITY_kFP
...
!** FLOW
    ...
    rComp%dImmed(rComp%iImmedSize) = 0.0_kFP
    rComp%zImmed(rComp%iImmedSize) = CMPLX(0.0_kFP, 0.0_kFP, KIND=kFP)
    rComp%iImmedSize               = rComp%iImmedSize - 1
...
!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    ...
    tReal           :: dnumber
    tByte           :: n
    tInteger        :: Ind1, Ind0, ib, in, nparams
    tLogical        :: bError
...
!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    tInteger    :: ifunctionLength, curiByteCodeSize, curiByteCodeSize2, curiImmedSize2
...
!** FLOW
    ...
    CALL AddCompiledByte(rComp, 0_kByte) ! Jump Index will be set below
    CALL AddCompiledByte(rComp, 0_kByte) ! Immed jump Index will be set below
    rComp%iStackPtr = rComp%iStackPtr - 1
...
!** FLOW
    ...
    ! Convert all nonzero double values to TrueVal and 0.0_kFP to
...
!** FLOW
    ...
    ! Convert all nonzero complex values to TrueVal and (0.0_kFP,0.0_kFP)
...
!** FLOW
    ...
    r = d * (3.141592653589793115997963468544185161590576171875_kFP / 180._kFP)
...
!** FLOW
    ...
    d = r * (180._kFP / 3.141592653589793115997963468544185161590576171875_kFP)
...
!** SUBROUTINE LOCAL VARIABLE DECLARATIONS:
    tInteger                :: iinstPtr, iStackPtr, idataPtr
    tInteger                :: ijumpAddr, iimmedAddr
    tReal                   :: daux
    tInteger                :: iaux
    tInteger, ALLOCATABLE   :: p_Irandom1(:), p_Irandom2(:)
...
!** FLOW
    ...
    DO WHILE(iinstPtr < rComp%iByteCodeSize)
        ...
        CASE (cAcos)
            IF ((Dstack(iStackPtr) < -1.0_kFP) .OR. &
                (Dstack(iStackPtr) >  1.0_kFP)) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
        ...
        CASE (cAcot)
            Dstack(iStackPtr) = ATAN(1.0_kFP/Dstack(iStackPtr))

        CASE (cAcoth)
            Dstack(iStackPtr) = ATANH(1.0_kFP/Dstack(iStackPtr))

        CASE (cAcosh)
            IF (Dstack(iStackPtr) < 1.0_kFP) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
        ...
        CASE (cAcsc)
            IF (Dstack(iStackPtr) > -Pi/2.0_kFP .AND.&
                Dstack(iStackPtr) <  Pi/2.0_kFP) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
            Dstack(iStackPtr) = ASIN(1.0_kFP/Dstack(iStackPtr))

        CASE (cAcsch)
            Dstack(iStackPtr) = ASINH(1.0_kFP/Dstack(iStackPtr))

        CASE (cAsec)
            IF ((Dstack(iStackPtr) > -1.0_kFP) .AND. &
                (Dstack(iStackPtr) <  1.0_kFP)) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
        ...
        CASE (cAsech)
            IF ((Dstack(iStackPtr) < 0.0_kFP) .OR. &
                (Dstack(iStackPtr) > 1.0_kFP)) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
            Dstack(iStackPtr) = ACOSH(1.0_kFP/Dstack(iStackPtr))

        CASE (cAsinh)
            IF ( Dstack(iStackPtr) <= 0._kFP) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
        ...
        CASE (cCot)
            Dstack(iStackPtr) = 1.0_kFP/TAN(Dstack(iStackPtr))

        CASE (cCoth)
            Dstack(iStackPtr) = 1.0_kFP/TANH(Dstack(iStackPtr))

        CASE (cCsc)
            Dstack(iStackPtr) = 1.0_kFP/SIN(Dstack(iStackPtr))

        CASE (cCsch)
            Dstack(iStackPtr) = 1.0_kFP/SINH(Dstack(iStackPtr))
        ...
        CASE (cSec)
            Dstack(iStackPtr) = 1.0_kFP/COS(Dstack(iStackPtr))

        CASE (cSech)
            Dstack(iStackPtr) = 1.0_kFP/COSH(Dstack(iStackPtr))

        CASE (cSign)
            Dstack(iStackPtr) = SIGN(1.0_kFP,Dstack(iStackPtr))
        ...
        CASE (cSqrt)
            IF (Dstack(iStackPtr) < 0.0_kFP) THEN
                EvalErrType = IllegalArgument
                dResult     = SYS_INFINITY_kFP
            END IF
    ...
    DO WHILE(iinstPtr < rComp%iByteCodeSize)
        ...
        CASE (cAtan2)
            IF (IsComplex(Zstack(iStackPtr-1)) .OR.&
                ...
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr-1) = CMPLX(ATAN2(REAL(Zstack(iStackPtr-1)),&
                                              REAL(zstack(iStackPtr))),0.0_kFP,KIND=kFP)
            iStackPtr = iStackPtr - 1

        CASE (cAcot)
            Zstack(iStackPtr) = ATAN(1.0_kFP/Zstack(iStackPtr))

        CASE (cAcoth)
            Zstack(iStackPtr) = ATANH(1.0_kFP/Zstack(iStackPtr))

        CASE (cAcosh)
            Zstack(iStackPtr) = ACOSH(Zstack(iStackPtr))

        CASE (cAcsc)
            Zstack(iStackPtr) = ASIN(1.0_kFP/Zstack(iStackPtr))

        CASE (cAcsch)
            Zstack(iStackPtr) = ASINH(1.0_kFP/Zstack(iStackPtr))

        CASE (cAsec)
            Zstack(iStackPtr) = ACOS(Zstack(iStackPtr))

        CASE (cAsech)
            Zstack(iStackPtr) = ACOSH(1.0_kFP/Zstack(iStackPtr))
        ...
        CASE (cAnint)
            IF (IsComplex(Zstack(iStackPtr))) THEN
              ! ANINT are not supported in complex-valued case
                ...
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr) = CMPLX(ANINT(AIMAG(zstack(iStackPtr))),0.0_kFP,KIND=kFP)
        ...
        CASE (cAint)
            IF (IsComplex(Zstack(iStackPtr))) THEN
              ! AINT are not supported in complex-valued case
                ...
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr) = CMPLX(AINT(AIMAG(zstack(iStackPtr))),0.0_kFP,KIND=kFP)

        CASE (cCeil)
            Zstack(iStackPtr) = CMPLX(CEILING(REAL(Zstack(iStackPtr))),&
                                      CEILING(AIMAG(Zstack(iStackPtr))),KIND=kFP)
        ...
        CASE (Ccmplx)
            ...
            Zstack(iStackPtr-1) = CMPLX(REAL(Zstack(iStackPtr-1)),&
                                        REAL(zstack(iStackPtr)),KIND=kFP)
            iStackPtr = iStackPtr - 1
        ...
        CASE (cCot)
            Zstack(iStackPtr) = 1.0_kFP/TAN(Zstack(iStackPtr))

        CASE (cCoth)
            Zstack(iStackPtr) = 1.0_kFP/TANH(Zstack(iStackPtr))

        CASE (cCsc)
            Zstack(iStackPtr) = 1.0_kFP/SIN(Zstack(iStackPtr))

        CASE (cCsch)
            Zstack(iStackPtr) = 1.0_kFP/SINH(Zstack(iStackPtr))
        ...
        CASE (cFloor)
            Zstack(iStackPtr) = CMPLX(FLOOR(REAL(Zstack(iStackPtr))),&
                                      FLOOR(AIMAG(Zstack(iStackPtr))),KIND=kFP)
        ...
        CASE (cImag)
            Zstack(iStackPtr) = CMPLX(AIMAG(Zstack(iStackPtr)),0.0_kFP,KIND=kFP)

        CASE (cLog)
            Zstack(iStackPtr) = LOG(Zstack(iStackPtr))

        CASE (cLog10)
            Zstack(iStackPtr) = LOG(Zstack(iStackPtr))/LOG(10.0_kFP)
        ...
        CASE (cReal)
            Zstack(iStackPtr) = CMPLX(REAL(Zstack(iStackPtr)),0.0_kFP,KIND=kFP)
        ...
        CASE (cSec)
            Zstack(iStackPtr) = 1.0_kFP/COS(Zstack(iStackPtr))

        CASE (cSech)
            Zstack(iStackPtr) = 1.0_kFP/COSH(Zstack(iStackPtr))
        ...
        CASE (cDiv)
            IF (Zstack(iStackPtr) == CMPLX(0.0_kFP,0.0_kFP,KIND=kFP)) THEN
                EvalErrType = DivisionByZero
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr-1) = Zstack(iStackPtr-1)/Zstack(iStackPtr)
        ...
        CASE (cMod)
            ...
            IF (AIMAG(Zstack(iStackPtr)) == 0.0_kFP) THEN
                EvalErrType = DivisionByZero
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr-1) = CMPLX(MOD(AIMAG(Zstack(iStackPtr-1)),&
                                            AIMAG(Zstack(iStackPtr))),0.0_kFP,KIND=kFP)
            iStackPtr = iStackPtr - 1
        ...
        CASE (cAnd)
            ...
            Zstack(iStackPtr-1) = CMPLX(LogcToDble(CmplToLogc(Zstack(iStackPtr-1)) .AND. &
                                                   CmplToLogc(Zstack(iStackPtr)) ),0.0_kFP,KIND=kFP)
            iStackPtr = iStackPtr - 1
        ...
        CASE (cNot)
            IF (IsComplex(Zstack(iStackPtr))) THEN
                ...
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr) = CMPLX(LogcToDble( .NOT. CmplToLogc(Zstack(iStackPtr)) ),0.0_kFP,KIND=kFP)
        ...
        CASE (cDeg)
            IF (IsComplex(Zstack(iStackPtr))) THEN
                ...
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr) = CMPLX(RadToDeg(REAL(Zstack(iStackPtr))),0.0_kFP,KIND=kFP)
        ...
        CASE (cRad)
            IF (IsComplex(Zstack(iStackPtr))) THEN
                ...
                zResult     = CMPLX(SYS_INFINITY_kFP,SYS_INFINITY_kFP,KIND=kFP)
            END IF
            Zstack(iStackPtr) = CMPLX(DegToRad(REAL(Zstack(iStackPtr))),0.0_kFP,KIND=kFP)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tInteger, PARAMETER :: up2low = IACHAR("a") - IACHAR("A")
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(ParserComponent), POINTER, INTENT(INOUT)   :: Array(:) ! array
    tInteger,                       INTENT(IN)      :: Size     ! size of array
...
END MODULE Class_MathExpParser
