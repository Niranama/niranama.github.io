...
!   *Macro - Basic Definitions.f90* file.  <br>
!   <br>
!^ **Important Note**: <br>
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'ModBase_PolyFloatNum'
    ! Size of character variable for storing error messages returned from
    ! ALLOCATE and DEALLOCATE statement.
    tInteger,  PARAMETER    :: MsgLen = 128
    ! typical constants
    tReal,     PARAMETER, PUBLIC    :: ZEROR = 0.0_kFP
    tReal,     PARAMETER, PUBLIC    :: ONER  = 1.0_kFP
    tReal,     PARAMETER, PUBLIC    :: PI_R  = 3.141592653589793238462643383279502884197_kFP
    tCmpx,     PARAMETER, PUBLIC    :: ZEROC = (ZEROR,ZEROR)
    tCmpx,     PARAMETER, PUBLIC    :: ONEC  = (ONER, ZEROR)
    tCmpx,     PARAMETER, PUBLIC    :: PI_C  = (PI_R, ZEROR)
    ! machine dependent parameters
    tReal,     PARAMETER, PUBLIC    :: MachEpsR = EPSILON(ONER)     ! machine epsilon
    tReal,     PARAMETER, PUBLIC    :: SmallR   = TINY(ONER)        ! the smallest positive number
    tReal,     PARAMETER, PUBLIC    :: LargeR   = HUGE(ONER)        ! the largest positive number
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tReal)
            BVal = -AVal
        TYPE IS (tCmpx)
            BVal = -REAL(AVal,KIND=kFP)
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (AVal)
        TYPE IS (tReal)
            BVal = -CMPLX(AVal,ZEROR,KIND=kFP)
        TYPE IS (tCmpx)
            BVal = -AVal
        END SELECT
    END SELECT
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tReal)
            BVal(1:N) = -AVal(1:N)
        TYPE IS (tCmpx)
            BVal(1:N) = -REAL(AVal(1:N),KIND=kFP)
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (AVal)
        TYPE IS (tReal)
            FORALL (I=1:N) BVal(I) = -CMPLX(AVal(I),ZEROR,KIND=kFP)
        TYPE IS (tCmpx)
            BVal(1:N) = -AVal(1:N)
        END SELECT
    END SELECT
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tCmpx)
            BVal(1:M,1:N) = -REAL(AVal(1:M,1:N),KIND=kFP)
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (AVal)
        TYPE IS (tReal)
            FORALL (I=1:M,J=1:N) BVal(I,J) = -CMPLX(AVal(I,J),ZEROR,KIND=kFP)
        TYPE IS (tCmpx)
            BVal(1:M,1:N) = -AVal(1:M,1:N)
        END SELECT
    END SELECT
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger, OPTIONAL, INTENT(IN)      :: Sign     !! if < 0, negative; otherwise, positive (default)
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tReal)
            AVal = Alpha*AVal
        TYPE IS (tCmpx)
            AVal = REAL(Alpha,KIND=kFP)*AVal
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (Alpha)
            TYPE IS (tReal)
            AVal = CMPLX(Alpha,ZEROR,KIND=kFP)*AVal
        TYPE IS (tCmpx)
            AVal = Alpha*AVal
        END SELECT
    END SELECT
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tCmpx)
            AVal(1:N) = REAL(Alpha,KIND=kFP)*AVal(1:N)
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (Alpha)
            TYPE IS (tReal)
            AVal(1:N) = CMPLX(Alpha,ZEROR,KIND=kFP)*AVal(1:N)
        TYPE IS (tCmpx)
            AVal(1:N) = Alpha*AVal(1:N)
        END SELECT
    END SELECT
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tCmpx)
            AVal(1:M,1:N) = REAL(Alpha,KIND=kFP)*AVal(1:M,1:N)
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (Alpha)
            TYPE IS (tReal)
            AVal(1:M,1:N) = CMPLX(Alpha,ZEROR,KIND=kFP)*AVal(1:M,1:N)
        TYPE IS (tCmpx)
            AVal(1:M,1:N) = Alpha*AVal(1:M,1:N)
        END SELECT
    END SELECT
...
! FLOW
    ...
    TYPE IS (tReal)
        ...
        TYPE IS (tCmpx)
            CVal(1:N) = REAL(BVal(1:N),KIND=kFP)*CVal(1:N)
        END SELECT
    TYPE IS (tCmpx)
        SELECT TYPE (BVal)
            TYPE IS (tReal)
            FORALL (I=1:N) CVal(I) = CMPLX(BVal(I),ZEROR,KIND=kFP)*CVal(I)
        TYPE IS (tCmpx)
            CVal(1:N) = BVal(1:N)*CVal(1:N)
        END SELECT
    END SELECT
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger, OPTIONAL, INTENT(IN)      :: ASign    !! if < 0, negative; otherwise, positive (default)
    tInteger, OPTIONAL, INTENT(IN)      :: BSign    !! if < 0, negative; otherwise, positive (default)
...
! FLOW
    ...
    TYPE IS (tCmpx)
        SELECT TYPE (AVal)
        TYPE IS (tReal)
            BVal(1:N) = CMPLX(AVal(1:N),ZEROR,KIND=kFP)
        TYPE IS (tCmpx)
            BVal(1:N) = AVal(1:N)
        END SELECT
    END SELECT
...
! FLOW
    ...
    TYPE IS (tCmpx)
        SELECT TYPE (AVal)
        TYPE IS (tReal)
            BVal(1:M,1:N) = CMPLX(AVal(1:M,1:N),ZEROR,KIND=kFP)
        TYPE IS (tCmpx)
            BVal(1:M,1:N) = AVal(1:M,1:N)
        END SELECT
    END SELECT
...
!** FLOW:
    ...
    IF (PRESENT(PDot)) THEN
        ...
        TYPE IS (tCmpx)
            SELECT TYPE (AVec)
            TYPE IS (tCmpx)
                Norm = Norm2_Complex(N,AVec,1)
            END SELECT
        END SELECT
    ELSE
        SELECT TYPE (AVec)
        TYPE IS (tReal)
            Norm = Norm2_Real(N,AVec,1)
        TYPE IS (tCmpx)
            Norm = Norm2_Complex(N,AVec,1)
        END SELECT
    END IF
...
CONTAINS
    ...
    FUNCTION Norm2_Complex(N,X,INCX) RESULT(Norm2)
        ...
        ELSE
            ...
            DO IX = 1,1 + (N-1)*INCX,INCX
                IF (REAL(X(IX),KIND=kFP) /= ZEROR) THEN
                    TEMP = ABS(REAL(X(IX),KIND=kFP))
                    IF (SCALE < TEMP) THEN
...
!** FLOW:
    ...
    CALL PolyMemAlloc(A, Mold, TrueVal)
...
!** FLOW:
    ...
    CALL PolyMemAlloc(Array, Size, Mold, TrueVal)
...
!** FLOW:
    ...
    CALL PolyMemAlloc(Array, Size1, Size2, Mold, TrueVal)
...
!** FLOW:
    ...
    CALL PolyMemAllocPtr(Ptr, Mold, TrueVal)
...
!** FLOW:
    ...
    CALL PolyMemAllocPtr1D(Ptr, Size, Mold, TrueVal)
...
!** FLOW:
    ...
    CALL PolyMemAllocPtr2D(Ptr, Size1, Size2, Mold, TrueVal)
...
!** FLOW:
    ...
    CALL PolyMemResize(Array, NewSize, CopyUserData)

    RETURN

CONTAINS

    FUNCTION CopyUserData(OutData,InData,N) RESULT(ErrStat)
        IMPLICIT NONE
        ...
        ErrStat = FalseVal
        CALL PolyAssign(OutData,InData,N)
    END FUNCTION CopyUserData
...
!** FLOW:
    ...
    CALL PolyMemResize(Array, NewSize1, NewSize2, CopyUserData)

    RETURN

CONTAINS

    FUNCTION CopyUserData(OutData,InData,N1,N2) RESULT(ErrStat)
        IMPLICIT NONE
        ...
        tLogical                :: ErrStat          ! true if error occurs while copying data
        ErrStat = FalseVal
        CALL PolyAssign(OutData,InData,N1,N2)
    END FUNCTION CopyUserData
...
!** FLOW:
    ...
    CALL PolyMemResizePtr1D(Ptr, NewSize, CopyUserData)
...
!** FLOW:
    ...
    CALL PolyMemResizePtr2D(Ptr, NewSize1, NewSize2, CopyUserData)
...
END MODULE ModBase_PolyFloatNum
