...
!** USE STATEMENTS:
    ...
    USE ModBase_PolyCommon,     ONLY: CopyData, CopyData_Real, CopyData_Complex
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tIndex                          :: ASize(1)
    PROCEDURE(CopyData), POINTER    :: MakeCopy => NULL()
    
!** FLOW:

    SELECT TYPE (Array)
    TYPE IS (tReal)
        MakeCopy => CopyData_Real
    TYPE IS (tCmpx)
        MakeCopy => CopyData_Complex
    END SELECT
    ASize(1) = NewSize
    CALL MemResize(Array, ASize, MakeCopy)
    NULLIFY(MakeCopy)
...
!** FLOW:
    ...
    END SELECT
    ASize(1) = NewSize
    CALL MemResizePtr(Ptr, ASize, MakeCopy)
    NULLIFY(MakeCopy)
