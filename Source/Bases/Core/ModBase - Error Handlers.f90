...
!   This module contains routines and parameters for error handling tasks.
!   The error messages are written to a (default) log file named 'XpfErrLog.txt'.
...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: ErrNone    = 0   !! no error level
    tInteger, PARAMETER :: ErrWarning = 1   !! warning error level
    tInteger, PARAMETER :: ErrSevere  = 2   !! severe error level
    tInteger, PARAMETER :: ErrFatal   = 3   !! fatal error level
    ! name of error file
    tCharParam          :: ErrFilename = 'XpfErrLog.txt'
...
!** FLOW
    ...
    ELSE
        ...
        ! first, check whether the 'XpfErrLog.txt' file is opened
        ...
        IF (FileOpened.AND.(IOS == 0)) THEN
            ...
            ! then write message to the 'XpfErrLog.txt' file
        ...
        ELSE
            ...
            IF (Existing) THEN
                ...
                ! write separation indicator to the 'XpfErrLog.txt' file
                WRITE(ErrorUnit, '(A)') '------------------------------------------------------------'
                WRITE(ErrorUnit, '(A)') ''
            ELSE
                OPEN(NEWUNIT=ErrorUnit, FILE=ErrFilename)
                ! write heading to the 'XpfErrLog.txt' file
...
!** FLOW
    ...
    ELSE
        ...
        IF (FileOpened.AND.(IOS == 0)) THEN
            ! get existing unit number for the opened file
            INQUIRE(FILE=ErrFilename, NUMBER=ErrorUnit)
            ! then close the 'XpfErrLog.txt' file
            CLOSE(ErrorUnit)
        END IF
    END IF
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger,  INTENT(IN)   :: Stat     !! error status
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger,  INTENT(IN)   :: Level    !! error level
