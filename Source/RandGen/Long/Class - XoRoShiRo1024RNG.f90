...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: SEED_SIZE = 16
    ! The coefficients for the jump function
    ! The coefficients for the long jump function
    tLong,    PARAMETER :: JUMP_COEFFICIENTS(0:SEED_SIZE-1) = [     &
        ToLong(Z'931197D8E3177F17'), ToLong(Z'B59422E0B9138C5F'),   &
        ToLong(Z'F06A6AFB49D668BB'), ToLong(Z'ACB8A6412C8A1401'),   &
        ToLong(Z'12304EC85F0B3468'), ToLong(Z'B7DFE7079209891E'),   &
        ToLong(Z'405B7EEC77D9EB14'), ToLong(Z'34EAD68280C44E4A'),   &
        ToLong(Z'E0E4BA3E0AC9E366'), ToLong(Z'8F46EDA8348905B7'),   &
        ToLong(Z'328BF4DBAD90D6FF'), ToLong(Z'C8FD6FB31C9EFFC3'),   &
        ToLong(Z'E899D452D4B67652'), ToLong(Z'45F387286ADE3205'),   &
        ToLong(Z'03864F454A8920BD'), ToLong(Z'A68FA28725B1B384')]
    tLong,    PARAMETER :: LONG_JUMP_COEFFICIENTS(0:SEED_SIZE-1) = [ &
        ToLong(Z'7374156360BBF00F'), ToLong(Z'4630C2EFA3B3C1F6'),    &
        ToLong(Z'6654183A892786B1'), ToLong(Z'94F7BFCBFB0F1661'),    &
        ToLong(Z'27D8243D3D13EB2D'), ToLong(Z'9701730F3DFB300F'),    &
        ToLong(Z'2F293BAAE6F604AD'), ToLong(Z'A661831CB60CD8B6'),    &
        ToLong(Z'68280C77D9FE008C'), ToLong(Z'50554160F5BA9459'),    &
        ToLong(Z'2FC20B17EC7B2A9A'), ToLong(Z'49189BBDC8EC9F8F'),    &
        ToLong(Z'92A65BCA41852CC1'), ToLong(Z'F46820DD0509C12A'),    &
        ToLong(Z'52B00C35FBF92185'), ToLong(Z'1E5B3B7F589E03C1')]
    ! Parameters for algorithm flag
    tInteger, PARAMETER, PUBLIC :: XoRoShiRo1024StarStar = 1    !! flag for XoRoShiRo1024StarStar PRNG
    tInteger, PARAMETER, PUBLIC :: XoRoShiRo1024Star     = 2    !! flag for XoRoShiRo1024Star PRNG
    tInteger, PARAMETER, PUBLIC :: XoRoShiRo1024PlusPlus = 3    !! flag for XoRoShiRo1024PlusPlus PRNG
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(LongRNG)  :: XoRoShiRo1024RNG
        PRIVATE
        ! states
        tLong       :: State(0:SEED_SIZE-1)
        ! index in "state" array
        tInteger    :: Index
        ! algorithm flag
        tInteger    :: XoRoShiRoAlgo = XoRoShiRo1024StarStar
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger,                INTENT(IN)     :: XoRoShiRoAlgo
    !% optional 32-bit integer seed(s)
    tLong, OPTIONAL,         INTENT(IN)     :: Seed(:)
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tLong, PARAMETER    :: Multiplier = ToLong(Z'9E3779B97F4A7C13')
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(XoRoShiRo1024RNG), INTENT(INOUT)  :: RNG          !! 'XoRoShiRo1024RNG' object
    tLong,                   INTENT(IN)     :: JCoef(0:)    !! jump coefficients

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tLong       :: NewState(0:SEED_SIZE-1)
    tLong       :: Dummy
    tInteger    :: I, J, B
...
! FLOW
    ...
    DO I = 0, SEED_SIZE-1
        DO B = 0, 63
            IF (IAND(JCoef(I), SHIFTL(1_kLong, B)) /= 0_kLong) THEN
                DO J = 0, SEED_SIZE-1
...
END MODULE Class_XoRoShiRo1024RNG
