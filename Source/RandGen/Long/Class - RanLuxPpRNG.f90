...
!** USE STATEMENTS:
    ...
    USE ModBase_Common
    USE ModBase_SIntUtil,   ONLY: MIN_I64, MAX_I64
    USE ModBase_UIntUtil,   ONLY: MAX_U64, ToUnsignedLong, UMulBasic
    USE Class_BaseRNG
    USE Class_IntegerRNG,   ONLY: MaskL
    USE Class_LongRNG
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(LongRNG)  :: RanLuxPpRNG
        ...
        tDouble     :: D(CACHE_SIZE) = 0.0_kDouble
        !% skipping parameter
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    tDouble, PARAMETER  :: Diff = ToDouble(MAX_I64) - ToDouble(MIN_I64)
...
! FLOW
    ...
    RandNum = ToLong(RNG%NextDouble()*Diff) + MIN_I64
...
CONTAINS
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        ...
        tDouble, TARGET, INTENT(OUT)    :: D(0:)    ! cached values (11 elements)
...
! FLOW
    ...
    GreaterM = (IOR(IOR(IOR(R(0), R(1)), R(2)), IAND(R(3), Mask)) > 0_kLong)
