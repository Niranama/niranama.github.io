...
! FLOW
    ...
    InitSeed = IOR(IAND(ToLong(IntSeeds(1)), MaskL), SHIFTL(ToLong(IntSeeds(2)), 32)) &
               + GOLDEN_RATIO_64
    ...
    CALL RNG%SetParameters(Mix_Murmur(ToInteger(SHIFTR(InitSeed, 32))), &
                           1, Mix_Lea(ToInteger(InitSeed)), &
                           Mix_Lea(ToInteger(InitSeed) + GOLDEN_RATIO_32))
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    tInteger, PARAMETER :: M = ToInteger(Z'ADB4A92D')
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    tInteger,         INTENT(IN)     :: S
    tInteger,         INTENT(IN)     :: X0
    tInteger,         INTENT(IN)     :: X1
...
! FLOW
    ...
    ! of the salt for 'A'
    CALL L32X64RNG_SetParameters(NewRNG, SHIFTL(ToInteger(Salt), 1), RNG%NextInteger(), &
