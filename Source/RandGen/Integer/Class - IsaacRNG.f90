...
!** MODULE PARAMETERS:
    ...
    tInteger, PARAMETER :: L_SIZE = 8
    ! Size of Rsl and Mem.
    tInteger, PARAMETER :: N_SIZE = SHIFTL(1, L_SIZE)
    ! Half-size of rsl() and mem().
    tInteger, PARAMETER :: H_SIZE = SHIFTA(N_SIZE, 1)
    ! For pseudo-random lookup.
    tInteger, PARAMETER :: MASK = SHIFTL(N_SIZE - 1, 2)
    ! The golden ratio.
    tInteger, PARAMETER :: GLD_RATIO = ToInteger(Z'9E3779B9')
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE, EXTENDS(IntegerRNG)  :: IsaccRNG
        ...
        tInteger    :: Rsl(0:N_SIZE-1)
        ! The internal state.
        tInteger    :: Mem(0:N_SIZE-1)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    tLong, PARAMETER :: CPar = ToLong(Z'000000006C078965')

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tInteger    :: Seed0(0:N_SIZE-1)
    tInteger    :: MinLen, SeedLen
    tIndex      :: J
    tLong       :: K
...
! FLOW
    ...
    IF (SeedLen < N_SIZE) THEN
        DO J = SeedLen, N_SIZE-1
            K = Seed0(J - SeedLen)
            Seed0(J) = ToInteger(IAND(CPar * IEOR(K, SHIFTA(K, 30)) + J, MaskL))
        END DO
    END IF
    RNG%Rsl = Seed0
