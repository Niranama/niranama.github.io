...
!** MODULE PARAMETERS:
  ...
  TYPE tComp
     ...
     REAL(FP),    DIMENSION(:), POINTER :: Immed
     INTEGER                            :: ImmedSize
     REAL(FP),    DIMENSION(:), POINTER :: Stack
     INTEGER                            :: StackSize, &
                                           StackPtr
  END TYPE tComp
...
!       STOP
  ...
  FUNCTION EvalF (i, Val) RESULT (res)
    ...
    IMPLICIT NONE
    INTEGER,                INTENT(IN) :: i                  ! Function identifier
    REAL(FP), DIMENSION(:), INTENT(IN) :: Val                ! Variable values
    REAL(FP)                           :: res                ! Result
    INTEGER                            :: IP,              & ! Instruction pointer
                                          DP,              & ! Data pointer
                                          SP                 ! Stack pointer
    REAL(FP),                PARAMETER :: zero = 0._FP
    !----- -------- --------- --------- --------- --------- --------- --------- -------
    DP = 1
    SP = 0
    DO IP=1,Comp(i)%ByteCodeSize
       ...
       CASE   (cDiv); IF (Comp(i)%Stack(SP)==0._FP) THEN; EvalErrType=1; res=zero; RETURN; ENDIF
       ...
       CASE (cLog10); IF (Comp(i)%Stack(SP)<=0._FP) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=LOG10(Comp(i)%Stack(SP))
       CASE   (cLog); IF (Comp(i)%Stack(SP)<=0._FP) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=LOG(Comp(i)%Stack(SP))
       CASE  (cSqrt); IF (Comp(i)%Stack(SP)<0._FP) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
       ...
       CASE  (cAsin); IF ((Comp(i)%Stack(SP)<-1._FP).OR.(Comp(i)%Stack(SP)>1._FP)) THEN
                      EvalErrType=4; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=ASIN(Comp(i)%Stack(SP))
       CASE  (cAcos); IF ((Comp(i)%Stack(SP)<-1._FP).OR.(Comp(i)%Stack(SP)>1._FP)) THEN
                      EvalErrType=4; res=zero; RETURN; ENDIF
