...
#define     KeyTypeA        REAL(KIND=kDP)
#define     KeyTypeB        REAL(KIND=kDP)
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'Class_HTabRealDP'
    tSInt64,   PARAMETER    :: INFVAL  = ToLong(Z'7FF0000000000000')  ! positive infinity
    tSInt64,   PARAMETER    :: NANVAL  = ToLong(Z'7FF8000000000000')  ! quiet NAN
    ! Special marker token used to indicate the deletion of a key-value pair
    tDouble,   PARAMETER    :: DELKEY = TRANSFER(INFVAL, 1.0_kDouble)
    ! Special marker token used to indicate the empty of a key-value pair
    tDouble,   PARAMETER    :: NULKEY = TRANSFER(NANVAL, 1.0_kDouble)
