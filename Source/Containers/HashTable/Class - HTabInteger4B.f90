...
!   *INTEGER(KIND=kI4B)* type as the type of its stored keys and an unlimited polymorphic type
...
#define     KeyTypeA        INTEGER(KIND=kI4B)
#define     KeyTypeB        INTEGER(KIND=kI4B)
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'Class_HTabInteger4B'
    ! Special marker token used to indicate the deletion of a key-value pair
    tSInt32,   PARAMETER    :: DELKEY = ToInteger(Z'7FFFFFFF')
    ! Special marker token used to indicate the empty of a key-value pair
    tSInt32,   PARAMETER    :: NULKEY = ToInteger(Z'80000000')
...
END MODULE Class_HTabInteger4B
