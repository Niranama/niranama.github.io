...
MODULE Class_ListAssignable

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *ListAssignable* type and related routines.
!   The *ListAssignable* type is a container with *CLASS(Assignable)* as the type
!   of its stored items.  It employs a conventional doubly-linked list
!   implementation.  All derived types extending from the *Assignable* type can be
!   used with this container. <br>
!   It should be noted that although the *ListAssignable* type allows items with
!   different types (any derived types that are in the *Assignable* class) to be
...
!   similar to the *ListAssignable* type but utilizes a different implementation.
!   Also, unlike the *ListAssignable* type, these container types are designed as
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'Class_ListAssignable'
...
!** DERIVED TYPE DEFINITIONS
    ...
    !   The type of the item stored in this node is a derived type in the *Assignable*
    !   class (i.e. the *Assignable* type or its subtypes).  The *DLLNode* type is a
    !   private type.
    TYPE DLLNode
        PRIVATE
        !> *Item* is an item (or value) stored in the node where its type can
        !   be any derived type that extends from the *Assignable* type.
        CLASS(Assignable), ALLOCATABLE  :: Item
    ...
    END TYPE DLLNode
    !> *ListAssignable* is a container type that employs a doubly-linked list implementation to
    ...
    !   The type of items stored in this container is a derived type in the *Assignable*
    !   class (i.e. the *Assignable* type or its subtypes).
    TYPE ListAssignable
        PRIVATE
...
!** INTERFACE DEFINITIONS:
    ...
    ABSTRACT INTERFACE
        ...
        FUNCTION IterFuncItem(Item,Done) RESULT(ErrStat)
            IMPORT
            CLASS(Assignable), INTENT(IN)       :: Item     ! item
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable),  INTENT(INOUT)   :: List         !! ListAssignable object
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(Assignable), OPTIONAL, INTENT(OUT)    :: Item     !! item of the removed node if requested
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    ...
    CLASS(Assignable), POINTER  :: CurrItem
    tLogical                    :: ErrStat
    tLogical                    :: Done
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(Assignable),     INTENT(IN)       :: Items(N) !! an array of items
...
!** PURPOSE OF THIS SUBROUTINE:
    !^ To destruct ListAssignable object.
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable), INTENT(INOUT)    :: List     !! ListAssignable object
    CLASS(Assignable),     INTENT(IN)       :: Item     !! item to be added to the list
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% ListAssignable object
    CLASS(ListAssignable), INTENT(INOUT)    :: List
    !% the one-based index into the list's items
    tIndex,                INTENT(IN)       :: Index
    !% the item to be added to the list
    CLASS(Assignable),     INTENT(IN)       :: Item
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(Assignable),     INTENT(OUT)      :: Item
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(ListAssignable),          INTENT(INOUT)   :: List
    !% the item to be removed from the list
    CLASS(Assignable), ALLOCATABLE, INTENT(INOUT)   :: Items(:)
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    ...
    CLASS(Assignable), POINTER      :: CurrItem
    CLASS(Assignable), ALLOCATABLE  :: IterItem
    tLogical                        :: Repeat
    tLogical                        :: IsTheEnd
    tLogical                        :: Success
    tInteger                        :: AllocStat
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable),       INTENT(INOUT)  :: List     !! ListAssignable object
    CLASS(Assignable), OPTIONAL, INTENT(OUT)    :: Item     !! first item
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable),       INTENT(INOUT)  :: List     !! ListAssignable object
    CLASS(Assignable), OPTIONAL, INTENT(OUT)    :: Item     !! last item
    tLogical                                    :: IsEmpty  !! true if the list is empty
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable),       INTENT(INOUT)  :: List     !! ListAssignable object
    CLASS(Assignable), OPTIONAL, INTENT(OUT)    :: Item     !! item of the next node
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable),       INTENT(INOUT)  :: List     !! ListAssignable object
    CLASS(Assignable), OPTIONAL, INTENT(OUT)    :: Item     !! item of the previous node
    tLogical                                    :: IsTheEnd !! true if the cursor has reached the end of the list
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable), INTENT(IN)   :: List !! ListAssignable object
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% ListAssignable object
    CLASS(ListAssignable), INTENT(IN)   :: List
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAssignable),          INTENT(INOUT)   :: List     !! ListAssignable object
    CLASS(Assignable), ALLOCATABLE, INTENT(OUT)     :: Items(:) !! an allocatable array of items
...
! FLOW
    ...
    IF (List%SameType()) THEN
        CALL MemAlloc(Items, N, List%Head%Item)
    ELSE
        ! set output
        Flag = FalseVal
        ! report error
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(ListAssignable), INTENT(INOUT) :: List  !! ListAssignable object
...
END MODULE Class_ListAssignable
