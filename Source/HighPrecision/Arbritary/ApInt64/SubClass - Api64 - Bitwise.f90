...
SUBMODULE (Class_ApInt64) SubClass_Api64_Bitwise
...
!** SUBMODULE PARAMETERS:
    ! name of the module
    tCharStar, PARAMETER    :: SubName = 'SubClass_Api64_Bitwise'
    ! parameters for large and small shifts
    tInteger,  PARAMETER    :: LargePos  = 6
    tInteger,  PARAMETER    :: SmallMask = 63
    tInteger,  PARAMETER    :: DigitBits = 64
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To shift the ApInt64 number left by the specified amount
...
!** FLOW
    ...
    ELSEIF (ShiftPos > 0) THEN
        ...
        OutVal = MakeCopy(InVal, InVal%Length+LargeShift+1_kIndex)
    ...
    ELSE
        CALL Handle_ErrLevel('ApInt64_ShiftLeft', SubName, ErrWarning, &
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To shift the ApInt64 number right by the specified amount
...
!** FLOW
    ...
    ELSEIF (ShiftPos == 0) THEN
        OutVal = MakeCopy(InVal)
    ELSE
        CALL Handle_ErrLevel('ApInt64_ShiftRight', SubName, ErrWarning, &
...
!** PURPOSE OF THIS SUBROUTINE:
    ...
    USE Class_ApInt64,  ONLY: SHIFTL, SHIFTR
#endif
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To shift the ApInt64 number left by the given amount (less than 64) starting
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To shift the ApInt64 number right by the given amount (less than 64).
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To shift the ApInt64 number right by 64*shift, i.e. moves each
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To return the bitwise logical complement of the ApInt64 object
...
!** FLOW
    ...
    IF (OutVal%Sign > 0) THEN
        OutVal%Sign = -1
        CALL UAddMag(OutVal, 1_kLong)
    ELSE
        OutVal%Sign = 1
        CALL USubMag(OutVal, 1_kLong)
    END IF
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To perform an inclusive OR on corresponding bits of the ApInt64 objects
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To perform a bitwise OR on corresponding bits of the ApInt64 objects

!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(ApInt64), INTENT(INOUT)    :: Big
    TYPE(ApInt64), INTENT(IN)       :: Mask
...
!** FLOW
    ...
    IF (Big%Sign > 0) THEN
        ...
        ELSE
            ...
            B = Mask%Digit(0)
            J = 1
            DO WHILE ((IOR(A, B) == 0_kLong).AND.(J < MLen))
                A = Big%Digit(J)
                B = Mask%Digit(J)
                J = J + 1
            END DO
            IF (A /= 0_kLong .AND. B == 0_kLong) THEN
                Big%Digit(J-1) = -A
                DO WHILE (Mask%Digit(J) == 0_kLong)
                    Big%Digit(J) = IEOR(Big%Digit(J), -1_kLong)
                    J = J + 1
                END DO
                IF (J < MLen) THEN
                ...
                END IF
                J = J + 1
            ELSEIF (A == 0_kLong) THEN    ! .AND. (B /= 0 || J == MLen)
                Big%Digit(J-1) = B
                DO WHILE ((J < MLen).AND.(Big%Digit(J) == 0_kLong))
            ...
            Big%Sign = -1
            Big%Length = Mask%Length
            DO WHILE (Big%Digit(Big%Length-1) == 0_kLong)
                Big%Length = Big%Length - 1
            END DO
        END IF
    ELSE
        ...
        IF (Mask%SIgn > 0) THEN
            IF (A /= 0_kLong .AND. B == 0_kLong) THEN
                DO WHILE ((J < MLen).AND.(Mask%Digit(J) == 0_kLong))
                    J = J + 1
                END DO
        ...
        END DO
    END IF
    DO WHILE ((Big%Digit(Big%Length-1) == 0_kLong).AND.(Big%Length > 1))
        Big%Length = Big%Length - 1
    END DO
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To perform a bitwise AND on corresponding bits of the ApInt64 objects
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To perform an exclusive OR on corresponding bits of the ApInt64 objects
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To perform a bitwise XOR on corresponding bits of the ApInt64 objects
    ...
    IF (Big%Sign > 0) THEN
        ...
        ELSE
            ...
            IF ((A /= 0_kLong).AND.(B == 0_kLong)) THEN
                ...
                END IF
                J = J + 1
            ELSEIF (A == 0_kLong) THEN    ! && (B /= 0 || J == mLen)
                Big%Digit(J-1) = B  ! -(0^-B)
            ELSE    ! A /= 0 && B /= 0
                Big%Digit(J-1) = -IEOR(A, -B)
                DO WHILE ((J < MLen).AND.(Big%Digit(J-1) == 0_kLong))
                    Big%Digit(J) = -IEOR(Big%Digit(J), NOT(Mask%Digit(J)))
                    J = J + 1
                END DO
                IF ((J  >= MLen).AND.(Big%Digit(J-1) == 0_kLong)) THEN
                    BLOCK
                        tLong, ALLOCATABLE  :: Tmp(:)
                        IF (J < Big%Length) THEN
                        ...
                        END IF
                        BLen = MAX(Big%Length, Mask%Length)
                        DO WHILE ((J < BLen).AND.(Tmp(J) == -1_kLong))
                            Big%Digit(J) = 0_kLong
                            J = J + 1
                        END DO
                        IF (BLen == SIZE(Big%Digit, KIND=kIndex)) THEN
    ...
    ELSE
        ...
        IF (Mask%Sign > 0) THEN
            ...
            ELSEIF (A == 0_kLong) THEN    ! && (B /= 0 || J == mLen)
                ...
                DO WHILE ((J < MLen).AND.(Big%Digit(J) == 0_kLong))
                    ...
                    J = J + 1
                END DO
                DO WHILE ((J < Big%Length).AND.(Big%Digit(J) == 0_kLong))
                    Big%Digit(J) = -1_kLong
                    J = J + 1
                END DO
                IF (J < MLen) THEN
            ...
            ELSE    ! A /= 0 && B /= 0
                ...
                IF (Big%Digit(J-1) == 0) THEN   ! Perform carry.
                    DO WHILE ((Big%Digit(J-1) == 0_kLong).AND.(J < MLen))
                        Big%Digit(J) = IEOR(Big%Digit(J), Mask%Digit(J))
                        Big%Digit(J) = Big%Digit(J) + 1
                        J = J + 1
                    END DO
                    BLen = MAX(Big%Length, Mask%Length)
                    DO WHILE ((Big%Digit(J-1) == 0_kLong).AND.(J < BLen))
                        Big%Digit(J) = Big%Digit(J) + 1_kLong
                        J = J + 1
                    END DO
                    IF (J == SIZE(Big%Digit, KIND=kIndex)) THEN
                        CALL MemResize(Big%Digit, J+2)
                    END IF
                    IF (Big%Digit(J-1) == 0_kLong) THEN
                        Big%Digit(J) = 1_kLong
                        Big%Length = J + 1
                        RETURN
                    END IF
        ...
        ELSE
            ...
            IF ((A /= 0_kLong).AND.(B == 0_kLong)) THEN
                Big%Digit(J-1) = -A
                DO WHILE (Mask%Digit(J) == 0_kLong)
                    Big%Digit(J) = IEOR(Big%Digit(J), -1_kLong)   ! ~Big%Digit(J)
                    J = J + 1
                END DO
                IF (J < Big%Length) THEN
                ...
                END IF
                J = J + 1
            ELSEIF (A == 0_kLong) THEN    ! && B /= 0
                Big%Digit(J-1) = -B
                DO WHILE ((J < Mask%Length).AND.(Big%Digit(J) == 0_kLong))
                    Big%Digit(J) = NOT(Mask%Digit(J))
                    J = J + 1
                END DO
                DO WHILE (Big%Digit(J) == 0_kLong)
...
!** PURPOSE OF THIS SUBROUTINE:
    ! To perform a bitwise AND and NOT on corresponding bits of the ApInt64 objects
...
!** FLOW
    ...
    IF (IsZero(LhsVal)) THEN
        OutVal = ZeroApInt64()
    ELSEIF (RhsVal == -OneApInt64()) THEN
        OutVal = ZeroApInt64()
    ELSE
        IF (IsZero(RhsVal)) THEN
            OutVal = MakeCopy(LhsVal, MAX(LhsVal%Length, RhsVal%Length)+2_kIndex)
            CALL Bitwise_AndNot(OutVal, ZeroApInt64())
        ELSE
            OutVal = MakeCopy(LhsVal, MAX(LhsVal%Length, RhsVal%Length)+2_kIndex)
    ...
    ELSE
        ...
        IF (Mask%Sign > 0) THEN
            ...
            IF (J < MLen) THEN
                ...
                IF ((J == MLen).AND.(Big%Digit(J-1) == 0_kLong)) THEN
                    BLen = MAX(Big%Length, Mask%Length)
                    DO WHILE ((J < BLen).AND.(Big%Digit(J) == -1_kLong))
                        Big%Digit(J) = 0_kLong    ! Mask%Digit(J) == Big%Digit(J)
                        J = J + 1
                    END DO
                    IF (J < BLen) THEN
                        Big%Digit(J) = -NOT(Big%Digit(J))
        ...
        ELSE
            ...
            IF ((A /= 0_kLong).AND.(B == 0_kLong)) THEN
                Big%Digit(J-1) = -A
                DO WHILE ((J < Mask%Length).AND.(Mask%Digit(J) == 0_kLong))
                    Big%Digit(J) = IEOR(Big%Digit(J), -1_kLong)
                    J = J + 1
                END DO
                IF (J < Big%Length) THEN
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tInteger    :: LargeBit, SmallBit
    tLong       :: J, K

!** FLOW

    IF (.NOT.ALLOCATED(InVal%Digit)) THEN
        CALL Handle_ErrLevel('ApInt64_SetBit', SubName, ErrSevere, &
    ...
    IF (OutVal%Sign > 0) THEN
        ...
        ELSEIF (LargeBit >= OutVal%Length) THEN
            DO WHILE (OutVal%Length <= LargeBit)
                OutVal%Digit(OutVal%Length) = 0_kLong
                OutVal%Length = OutVal%Length + 1_kIndex
            END DO
        END IF
        OutVal%Digit(LargeBit) = IOR(OutVal%Digit(LargeBit), SHIFTL(1_kLong, SmallBit))
    ELSE
        IF (LargeBit >= OutVal%Length) RETURN
        J = 0_kLong
        DO WHILE ((J <= LargeBit).AND.(OutVal%Digit(J) == 0_kLong))
            J = J + 1_kLong
        END DO
        IF (J > LargeBit) THEN
            OutVal%Digit(LargeBit) = SHIFTL(-1_kLong, SmallBit)
            DO WHILE (OutVal%Digit(J) == 0_kLong)
                OutVal%Digit(J) = -1_kLong
                J = J + 1_kLong
            END DO
            OutVal%Digit(J) = NOT(-OutVal%Digit(J))
            IF ((J == OutVal%Length-1).AND.(OutVal%Digit(OutVal%Length-1) == 0_kLong)) THEN
                OutVal%Length = OutVal%Length - 1_kIndex
            END IF
            RETURN
        END IF
        IF (J < LargeBit) THEN
            OutVal%Digit(LargeBit) = IAND(OutVal%Digit(LargeBit), NOT(SHIFTL(1_kLong, SmallBit)))
            DO WHILE (OutVal%Digit(OutVal%Length-1) == 0_kLong)
                OutVal%Length = OutVal%Length - 1_kIndex
            END DO
            RETURN
        END IF
        J = TRAILZ(OutVal%Digit(J))
        K = SHIFTL(1_kLong, SmallBit)
        IF (K -J > 0_kLong) THEN
            OutVal%Digit(LargeBit) = IAND(OutVal%Digit(LargeBit), NOT(K)) ! Unsigned compare.
        ELSE
            OutVal%Digit(LargeBit) = IEOR(OutVal%Digit(LargeBit), IEOR(SHIFTL(J, 1_kLong) - 1_kLong, (K-1_kLong)))
...
!** FLOW
    ...
    IF (.NOT.ALLOCATED(InVal%Digit)) THEN
        CALL Handle_ErrLevel('ApInt64_ClearBit', SubName, ErrSevere, &
                          'Empty Input : Storage of InVal has not yet been allocated.')
        RETURN
    END IF
    ...
    IF (OutVal%Sign > 0) THEN
        IF (LargeBit<OutVal%Length) THEN
            OutVal%Digit(LargeBit) = IAND(OutVal%Digit(LargeBit), NOT(SHIFTL(1_kLong, SmallBit)))
            DO WHILE ((OutVal%Digit(OutVal%Length-1) == 0_kLong).AND.(OutVal%Length > 1_kIndex))
                OutVal%Length = OutVal%Length - 1_kIndex
            END DO
        END IF
    ELSE
        IF (LargeBit>=SIZE(OutVal%Digit)) THEN
            ...
            OutVal%Digit(LargeBit) = IOR(OutVal%Digit(LargeBit), SHIFTL(1_kLong, SmallBit))
            RETURN
        ELSE IF (LargeBit>=OutVal%Length) THEN
            DO WHILE (OutVal%Length <= LargeBit)
                OutVal%Digit(OutVal%Length) = 0_kLong
                OutVal%Length = OutVal%Length + 1_kIndex
            END DO
        ...
        J = TRAILZ(OutVal%Digit(J))
        K = SHIFTL(1_kLong, SmallBit)
        IF (J-K > 0_kLong) RETURN     ! Unsigned compare
        IF (J-K < 0_kLong) THEN
            OutVal%Digit(LargeBit) = IOR(OutVal%Digit(LargeBit), K)
            RETURN
        END IF
        J = OutVal%Digit(LargeBit)
        IF (J == IEOR(-1_kLong, K-1_kLong)) THEN
            OutVal%Digit(LargeBit) = 0_kLong
             J = LargeBit+1_kLong
            DO WHILE ((J < OutVal%Length).AND.(OutVal%Digit(J) == -1_kLong))
                OutVal%Digit(J) = 0_kLong
                J = J + 1_kLong
            END DO
            IF (J == SIZE(OutVal%Digit)) CALL MemResize(OutVal%Digit, ToIndex(J)+2_kIndex)
            IF (J == OutVal%Length) THEN
                OutVal%Digit(OutVal%Length) = 1_kLong
                OutVal%Length = OutVal%Length + 1_kIndex
                RETURN
            END IF
            OutVal%Digit(J) = -NOT(OutVal%Digit(J))
        ELSE
            J = TRAILZ(IEOR(J, IEOR(-1_kLong, K-1_kLong)))
            OutVal%Digit(LargeBit) = IEOR(OutVal%Digit(LargeBit), IOR(J, IEOR(J-1_kLong, K-1_kLong)))
        END IF
    END IF
...
!** FLOW
    ...
    IF (.NOT.ALLOCATED(InVal%Digit)) THEN
        CALL Handle_ErrLevel('ApInt64_FlipBit', SubName, ErrSevere, &
                          'Empty Input : Storage of InVal has not yet been allocated.')
        RETURN
    END IF
    ...
    IF (LargeBit >= SIZE(OutVal%Digit)) THEN
        CALL MemResize(OutVal%Digit, LargeBit+1_kIndex)
        OutVal%Length = LargeBit + 1_kIndex
        OutVal%Digit(LargeBit) = IEOR(OutVal%Digit(LargeBit), SHIFTL(1_kLong, SmallBit))
    ELSEIF (LargeBit >= OutVal%Length) THEN
        DO WHILE (OutVal%Length <= LargeBit)
            OutVal%Digit(OutVal%Length) = 0_kLong
            OutVal%Length = OutVal%Length + 1_kIndex
    ...
    ELSE
        ...
        ELSE
            ...
            K = SHIFTL(1_kLong, SmallBit)
            IF (J-K > 0_kLong) THEN
                OutVal%Digit(LargeBit) = IEOR(OutVal%Digit(LargeBit), IEOR(SHIFTL(J, 1) - 1_kLong, K-1_kLong))
                RETURN
            END IF
            IF (J-K < 0_kLong) THEN
            ...
            END IF
            J = OutVal%Digit(LargeBit)
            IF (J == IEOR(-1_kLong, K-1_kLong)) THEN    ! TODO: Refactor with clearBit?
                OutVal%Digit(LargeBit) = 0_kLong
                J = LargeBit + 1_kLong
                DO WHILE ((J < OutVal%Length).AND.(OutVal%Digit(J) == -1_kLong))
                    OutVal%Digit(J) = 0_kLong
...
!** FLOW
    ...
    IF (.NOT.ALLOCATED(Big%Digit)) THEN
        Flag = FalseVal
        CALL Handle_ErrLevel('ApInt64_TestBit', SubName, ErrWarning, &
                          'Empty Input : Storage of InVal has not yet been allocated.')
        RETURN
    END IF
    ...
    END IF
    IF (Big%Sign > 0) THEN
        Flag = (IAND(Big%Digit(LargeBit), SHIFTL(1_kLong, SmallBit)) /= 0_kLong)
        RETURN
    END IF

    J = 0_kLong
    DO WHILE ((J <= LargeBit).AND.(Big%Digit(J) == 0_kLong))
        J = J + 1_kLong
    END DO
    IF (J > LargeBit) THEN
        Flag = FalseVal
    ...
    ELSE
        ...
        Flag = (IAND(J, SHIFTL(1_kLong, SmallBit)) /= 0_kLong)
    END IF
...
END SUBMODULE SubClass_Api64_Bitwise
